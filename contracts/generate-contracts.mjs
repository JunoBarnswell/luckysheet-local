...
const tsMutations = entries.map(([id, capability]) =>
  `  ${JSON.stringify(id)}: { durability: ${JSON.stringify(capability.durability)}, remote: ${Boolean(capability.remote)}, schema: ${JSON.stringify(capability.schema)}, minRole: ${JSON.stringify(capability.minRole)}, rebasePolicy: ${JSON.stringify(capability.rebasePolicy)}, javaReducer: ${Boolean(capability.javaReducer)}${capability.replacement ? `, replacement: ${JSON.stringify(capability.replacement)}` : ''}${capability.collaborationKind ? `, collaborationKind: ${JSON.stringify(capability.collaborationKind)}` : ''} },`,
).join('\n');

...
`sheet.rename` was listed as a Java structural planner mutation but omitted from the required-patch contract; its Java reducer rewrote formula owners and returned no patch. The reducer now emits the existing v5 reversible cell/rule/formula-object/defined-name facts from the same rewrite, the generated protocol requires those facts for `sheet.rename`, and repeatable-history replay can backfill legacy rename events. ACK preflight applies those authoritative facts and installs them in local undo/redo history instead of comparing them with the TypeScript rename transform's incomplete delta set. Remote, undo, and redo replay now change only worksheet identity before applying the server facts; calculation context is conservatively rebuilt for that identity-only replay. Formula-rule owners must have stable identity/ranges, and preserved-only formula owners still fail closed. Regression coverage was added but not run locally in this static-review phase. Local optimistic command execution still computes its own rename transform before server ACK; the full owner index/patch contract remains incomplete, as do structural collaboration and opaque OOXML reference handling.

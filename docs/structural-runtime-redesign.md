...
4. **图表投影索引全工作簿重建：** `ProjectionRuntime.invalidateProjectionMutations` 对结构 mutation 将 `chartSourceIndexDirty` 置位，紧接着 `invalidateDependentChartProjections` 调用 `ensureChartSourceIndex`，遍历所有 sheets 和 drawing payloads 重建图表 source postings。此索引与公式依赖索引职责不同，不能仅因分开维护判错；确认的问题是每次结构 mutation 都产生 `O(all sheets + all drawings)` 扫描，违反目标中的 `O(affected references + affected objects + moved cells)`。方案：让 canonical typed owner index 提供受影响图表 owner delta，或按 patch 增量更新图表 source postings，投影缓存只消费这些 owner delta。

...
public class WorkspaceService {
    ...
    public WorkspaceSpaceEntity ensurePersonalSpace(String subject) {
        return spaces.findByOwnerSubjectAndType(subject, WorkspaceSpaceType.PERSONAL).orElseGet(() -> {
            Instant now = Instant.now();
            WorkspaceSpaceEntity space = new WorkspaceSpaceEntity(UUID.randomUUID().toString(), "我的云文档",
    ...
    public void deleteFolder(String spaceId, String folderId, String subject) {
        ...
        if (!folders.findBySpaceIdOrderByName(spaceId).stream().filter(item -> folderId.equals(item.getParentId())).toList().isEmpty()) {

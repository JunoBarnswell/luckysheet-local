...
select
    (select count(*) from workbooks w where not exists (
        ...
        where a.source_kind = 'WORKBOOK' and a.unit_id = w.unit_id
          ...
          and a.source_checksum = lower(rawtohex(hash('SHA-256', stringtoutf8(w.snapshot_json))))
          and a.source_checksum = lower(rawtohex(hash('SHA-256', stringtoutf8(a.snapshot_json))))
          ...
          and m.checksum = lower(rawtohex(hash('SHA-256', stringtoutf8(m.manifest_json))))
    ...
    + (select count(*) from snapshot_checkpoint c where not exists (
        ...
        where a.source_kind = 'CHECKPOINT' and a.unit_id = c.unit_id and a.revision = c.revision
          and a.source_checksum = lower(rawtohex(hash('SHA-256', stringtoutf8(c.snapshot_json))))
          and a.source_checksum = lower(rawtohex(hash('SHA-256', stringtoutf8(a.snapshot_json))))
    ...
    + (select count(*) from operation_log o where not exists (
        select 1 from workbook_v10_operation_archive a where a.operation_id = o.operation_id
          ...
          and a.source_checksum = lower(rawtohex(hash('SHA-256', stringtoutf8(o.envelope_json))))
          and a.source_checksum = lower(rawtohex(hash('SHA-256', stringtoutf8(a.envelope_json))))
    ))
    + (select count(*) from workbook_native_artifact_migration_archive a where not exists (
        ...
        where n.unit_id = a.unit_id and n.workbook_revision = w.revision
          ...
          and a.checksum = lower(rawtohex(hash('SHA-256', a.content)))

...
select 'WORKBOOK', unit_id, revision, snapshot_revision, snapshot_json, encode(sha256(convert_to(snapshot_json, 'UTF8')), 'hex'),
       null, created_at, updated_at, current_timestamp
from workbooks;
insert into workbook_v10_migration_archive
    (source_kind, unit_id, revision, snapshot_revision, snapshot_json, source_checksum,
     original_checksum, source_created_at, source_updated_at, archived_at)
select 'CHECKPOINT', unit_id, revision, revision, snapshot_json, encode(sha256(convert_to(snapshot_json, 'UTF8')), 'hex'),
...
select operation_id, unit_id, revision, actor_subject, client_sequence, base_revision,
       envelope_json, encode(sha256(convert_to(envelope_json, 'UTF8')), 'hex'), committed_at
from operation_log;

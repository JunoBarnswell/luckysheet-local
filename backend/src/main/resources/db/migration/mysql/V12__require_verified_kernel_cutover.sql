...
select
    (select count(*) from workbooks w where not exists (
        ...
        where a.source_kind = 'WORKBOOK' and a.unit_id = w.unit_id
          ...
          and a.source_checksum = lower(sha2(convert(w.snapshot_json using utf8mb4), 256))
          and a.source_checksum = lower(sha2(convert(a.snapshot_json using utf8mb4), 256))
          ...
          and m.checksum = lower(sha2(convert(m.manifest_json using utf8mb4), 256))
    ...
    + (select count(*) from snapshot_checkpoint c where not exists (
        ...
        where a.source_kind = 'CHECKPOINT' and a.unit_id = c.unit_id and a.revision = c.revision
          and a.source_checksum = lower(sha2(convert(c.snapshot_json using utf8mb4), 256))
          and a.source_checksum = lower(sha2(convert(a.snapshot_json using utf8mb4), 256))
    ...
    + (select count(*) from operation_log o where not exists (
        select 1 from workbook_v10_operation_archive a where a.operation_id = o.operation_id
          ...
          and a.source_checksum = lower(sha2(convert(o.envelope_json using utf8mb4), 256))
          and a.source_checksum = lower(sha2(convert(a.envelope_json using utf8mb4), 256))
    ))
    + (select count(*) from workbook_native_artifact_migration_archive a where not exists (
        ...
        where n.unit_id = a.unit_id and n.workbook_revision = w.revision
          ...
          and a.checksum = lower(sha2(a.content, 256))
...
alter table workbooks drop column snapshot_json, drop column snapshot_revision;

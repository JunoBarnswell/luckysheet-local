...
[
    {
        ...
        "table": [], //结构化表（可选）：{ id, name, range, columns, tableStyleId }
        "note": null, //工作表级 Note（与 cell.ps 批注并存）
        "notes": {}, //单元格 Note 稀疏表："r_c" -> { text }
        "definedNames": [] //本表定义名称（可选）
    },
    {
        "name": "Sheet2",
        "color": "",
...
- 类型：Array
- 默认值：`[]`
- 作用：结构化 Table（对照 Univer OSS sheets-table，**不是**交替色）。元素字段：
  + `{String} id`
  + `{String} name`
  + `{Object} range`：`{ row:[r1,r2], column:[c1,c2] }`
  + `{Array} columns`：`{ id, name, index }`
  + `{String} tableStyleId`

------------
### note / notes
- 类型：Object
- 作用：工作表级 `note` 与单元格稀疏 `notes["r_c"]`。与批注 `cell.ps` **并存**，见仓库 `docs/cell-note-vs-postil.md`。

------------
### definedNames
- 类型：Array
- 默认值：`[]`
- 作用：本表定义名称。也可在工作簿 `options.definedNames` 提供。超链接类型 `DEFINE_NAME` 会解析这些名称以及 `table[].name`。

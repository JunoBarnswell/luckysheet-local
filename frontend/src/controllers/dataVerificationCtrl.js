...
const dataVerificationCtrl = createContextualModule('dataVerificationCtrl', {
        ...
        let content =  `<div class="box">
                            ...
                            <div class="box-item">
                                ...
                                <select id="data-verification-type-select">
                                    ...
                                    <option value="any">${dvText.any || "ANY"}</option>
                                    <option value="custom">${dvText.custom || "CUSTOM"}</option>
                                </select>
                                <div class="show-box">
                                    ...
                                    <div class="show-box-item show-box-item-any">
                                        <div class="txt">${dvText.any || "ANY"}</div>
                                    </div>
                                    <div class="show-box-item show-box-item-custom">
                                        <div class="input input2">
                                            <input type="text" class="data-verification-value1" placeholder="${dvText.placeholderCustom || "=A1>0"}"/>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    init: function(){
        ...
        $(document).off("change.typeSelect").on("change.typeSelect", "#data-verification-type-select", function(e) {
            ...
            else if(value == 'any' || value == 'ANY'){
                $("#luckysheet-dataVerification-dialog .show-box .show-box-item-any").show();
            }
            else if(value == 'custom' || value == 'CUSTOM'){
                $("#luckysheet-dataVerification-dialog .show-box .show-box-item-custom").show();
                let value1 = "";
                if(item.type == 'custom' || item.type == 'CUSTOM'){
                    value1 = item.value1;
                }
                $("#luckysheet-dataVerification-dialog .show-box-item-custom .data-verification-value1").val(value1);
            }
        });
        ...
        $(document).off("click.dvSaveConfirm").on("click.dvSaveConfirm", "#luckysheet-dataVerification-dialog-confirm", function(e) {
            ...
            if(type == 'dropdown'){
                ...
                type = 'custom';
                value1 = $("#luckysheet-dataVerification-dialog .show-box-item-custom .data-verification-value1").val().trim();
                if(value1.length == 0){
                    tooltip.info('<i class="fa fa-exclamation-triangle"></i>', dvText.tooltipInfoCustom || dvText.tooltipInfo5);
                    return;
                }
            }
    ...
    dataAllocation: function(){
        ...
        let dataVerification = normalizeDataVerificationMap($.extend(true, {}, _this.dataVerification)); 
        ...
        else if(item.type == 'any' || item.type == 'ANY'){
            $("#luckysheet-dataVerification-dialog .show-box .show-box-item-any").show();
        }
        else if(item.type == 'custom' || item.type == 'CUSTOM'){
            $("#luckysheet-dataVerification-dialog .show-box .show-box-item-custom").show();
            $("#luckysheet-dataVerification-dialog .show-box-item-custom .data-verification-value1").val(item.value1);
        }
    ...
    cellFocus: function(r, c, clickMode){
        ...
        let item = ensureLsItem(_this.dataVerification[r + '_' + c]);
        ...
        //单元格数据验证 类型是 下拉列表（含 LIST_MULTIPLE）
        if(item.type == 'dropdown' || item.type == 'list_multiple' || item.type == 'LIST_MULTIPLE'){
    ...
    validateCellData: function(cellValue, item){
        ...
        if(type == 'custom' || type == 'CUSTOM'){
            let formulaTxt = value1 == null ? "" : String(value1).trim();
            if(formulaTxt == ""){
                return true;
            }
            if(formulaTxt.charAt(0) != "="){
                formulaTxt = "=" + formulaTxt;
            }
            try {
                let funcV = formula.execfunction(formulaTxt);
                let v = funcV && funcV[1];
                if(typeof v != "boolean"){
                    v = !!Number(v);
                }
        ...
        if(type == 'list_multiple' || type == 'LIST_MULTIPLE'){
            type = 'dropdown';
            type2 = true;
        }
    ...
    dropdownListShow: function(){
        ...
        let item = ensureLsItem(_this.dataVerification[rowIndex + '_' + colIndex]);
        let list = _this.getDropdownList(item.value1);

        let optionHtml = '';
        if ((item.type === 'dropdown' && item.type2) || item.type === 'list_multiple' || item.type === 'LIST_MULTIPLE') {
            // 下拉多选的情况下 将已经选择的标出来

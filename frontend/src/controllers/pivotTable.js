...
import { cloneSheetData } from "../global/sparseGrid";
...
const pivotTable = createContextualModule("pivotTable", {
    ...
    celldata: null,
    origindata: null,
    normalizePivotConfig: function(raw) {
        if (raw == null) {
            return null;
        }
        if (getObjType(raw) == "object") {
            return raw;
        }
        try {
    ...
    isEmptyPivotResult: function(ret) {
        return !ret || ret.length === 0 || ret[0] == null || ret[0].length === 0;
    },
    hasPivotConfigUi: function() {
        return (
            $("#luckysheet-modal-dialog-config-row .luckysheet-modal-dialog-slider-config-item").length +
                $("#luckysheet-modal-dialog-config-column .luckysheet-modal-dialog-slider-config-item").length +
                $("#luckysheet-modal-dialog-config-value .luckysheet-modal-dialog-slider-config-item").length +
                $("#luckysheet-modal-dialog-config-filter .luckysheet-modal-dialog-slider-config-item").length >
            0
        );
    },
    getCellData: function(cursheetindex, datasheetindex, data_select_save) {
        ...
        let sheetIndex = cursheetindex != null ? cursheetindex : Store.currentSheetIndex;
        let realIndex = getSheetIndex(sheetIndex);

        if (realIndex == null || Store.luckysheetfile[realIndex] == null) {
            _this.column = null;
            _this.row = null;
            _this.values = null;
            _this.filter = null;
        ...
        let file = Store.luckysheetfile[realIndex];
        let pivotCfg = _this.normalizePivotConfig(file.pivotTable);
        file.pivotTable = pivotCfg == null ? file.pivotTable : pivotCfg;

        if (pivotCfg != null) {
            _this.column = pivotCfg.column;
            _this.row = pivotCfg.row;
            _this.values = pivotCfg.values;
            _this.filter = pivotCfg.filter;
            _this.showType = pivotCfg.showType;
            _this.filterparm = pivotCfg.filterparm;
            _this.drawPivotTable = pivotCfg.drawPivotTable != null ? pivotCfg.drawPivotTable : true;
            _this.pivotTableBoundary = pivotCfg.pivotTableBoundary != null ? pivotCfg.pivotTableBoundary : [12, 6];
            _this.pivot_select_save = data_select_save != null ? data_select_save : pivotCfg.pivot_select_save;
            _this.pivotDataSheetIndex = datasheetindex != null ? datasheetindex : pivotCfg.pivotDataSheetIndex;
        } else {
            _this.column = null;
            _this.row = null;
            _this.values = null;
            ...
            _this.drawPivotTable = true;
            _this.pivotTableBoundary = [12, 6];
            _this.pivot_select_save = data_select_save != null ? data_select_save : Store.luckysheet_select_save;
            _this.pivotDataSheetIndex = datasheetindex != null ? datasheetindex : sheetIndex;
        }

        let pivotrealIndex = getSheetIndex(_this.pivotDataSheetIndex);
        let otherfile = pivotrealIndex != null ? Store.luckysheetfile[pivotrealIndex] : null;
        ...
        _this.origindata =
            _this.pivot_select_save != null ? getdatabyselectionD(otherfile.data, _this.pivot_select_save) || [] : [];
        ...
        _this.pivot_data_type = {};
        if (_this.celldata.length > 1 && _this.celldata[1] != null) {
    ...
    setDatatojsfile: function(attr, value, cindex) {
        ...
        if (index == null || Store.luckysheetfile[index] == null) {
            _this[attr] = value;
            return;
        }
    ...
    changePivotTable: function(index) {
        ...
        let filePos = getSheetIndex(index);
        let file = filePos != null ? Store.luckysheetfile[filePos] : null;
        let pivotCfg = file != null ? _this.normalizePivotConfig(file.pivotTable) : null;
        if (file != null && pivotCfg != null) {
            file.pivotTable = pivotCfg;
        }
        ...
        let pivotDataSheetIndex = pivotCfg.pivotDataSheetIndex;
    ...
    refreshPivotTable: function(isRefreshCanvas = true) {
        ...
        // 源表可能已改：每次重算都从 pivotDataSheetIndex 重读，避免沿用过期 celldata
        let pivotSheet = _this.pivotSheetIndex != null ? _this.pivotSheetIndex : Store.currentSheetIndex;
        _this.getCellData(pivotSheet, _this.pivotDataSheetIndex, _this.pivot_select_save);

        // 配置面板未挂载时不要把 column/row/values 写成空数组，否则切 sheet 会把已有透视配置冲掉
        if (_this.hasPivotConfigUi()) {
        ...
        if (_this.isEmptyPivotResult(ret)) {
        ...
        redo["curdata"] = cloneSheetData(data);
    ...
    getPivotTableData: function(dataindex) {
        ...
        let pivotTableCfg = this.normalizePivotConfig(Store.luckysheetfile[index]["pivotTable"]);
        if (pivotTableCfg == null) {
            return {};
        }

        return $.extend(true, {}, pivotTableCfg);
    },
    ...
    dataHandler: function(column, row, values, showType, celldata) {
        ...
        if ((column.length == 0 && row.length == 0 && values.length == 0) || celldata.length == 0) {
            _this.pivotDatas = [];
            // 返回 [[]]：调用方（含 sheetmanage.refreshPivotTableByFile）读 ret[0].length 时不会空指针
            return [[]];
        }

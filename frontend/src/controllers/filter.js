...
import { getFilterValueInfo, shouldHideByCaljs, sortFilterValueKeys, FILTER_CUSTOM_OPERATORS } from './filterCondition';
...
function labelFilterOptionState($top, optionstate, rowhidden, caljs, notSave, str, edr, cindex, stc, edc) {
    if (optionstate) {
        ...
        if (caljs != null) {
            ...
            if (caljs["and"] != null) {
                $top.data("byconditionand", caljs["and"]);
            }

            if (caljs["operator1"] != null) {
                $top.data("byconditionoperator1", caljs["operator1"]);
            }

            if (caljs["operator2"] != null) {
                $top.data("byconditionoperator2", caljs["operator2"]);
            }
        }
    }
    else {
        ...
        $top.data("byconditionvalue", "null").data("byconditiontype", "0").data("byconditiontext", "无").data("byconditionvalue1", "").data("byconditionvalue2", "").data("byconditionand", "").data("byconditionoperator1", "").data("byconditionoperator2", "");
    }
...
function createFilter() {
    ...
    if(Store.luckysheet_select_save.length > 1){
        ...
        focusEditor(document.getElementById(Store.container), { preventScroll: true });
...
function createFilterOptions(luckysheet_filter_save, filterObj) {
    ...
    for (let c = c1; c <= c2; c++) {
        ...
        if(filterObj == null || filterObj[c - c1] == null){
            ...
            if(filterObj[c - c1].caljs != null){
                ...
                let caljs_and_data = '';
                if (filterObj[c - c1].caljs["and"] != null) {
                    caljs_and_data = 'data-byconditionand="'+ filterObj[c - c1].caljs["and"] +'" ';
                }

                let caljs_operator1_data = '';
                if (filterObj[c - c1].caljs["operator1"] != null) {
                    caljs_operator1_data = 'data-byconditionoperator1="'+ filterObj[c - c1].caljs["operator1"] +'" ';
                }

                let caljs_operator2_data = '';
                if (filterObj[c - c1].caljs["operator2"] != null) {
                    caljs_operator2_data = 'data-byconditionoperator2="'+ filterObj[c - c1].caljs["operator2"] +'" ';
                }
                ...
                caljs_data = 'data-caljs="'+ JSON.stringify(filterObj[c - c1].caljs) +'" ' +
                                 ...
                                 caljs_value1_data + caljs_value2_data + caljs_and_data + caljs_operator1_data + caljs_operator2_data;
            }
            else{
                caljs_data = '';
            }
    ...
    Store.luckysheet_filter_save = luckysheet_filter_save;
}

function applyFilterAfterRowColumnOp() {
    const file = Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)];
    if (file == null) {
        return;
    }
    if (file.filter_select != null && JSON.stringify(file.filter_select) != "{}") {
        Store.luckysheet_filter_save = file.filter_select;
    }
    else {
        Store.luckysheet_filter_save = { "row": [], "column": [] };
    }
}

function dateSerialFromFilterInput(value) {
    const generated = genarate(value);
    return generated != null ? generated[2] : value;
}

function fillFilterCustomOperators($menu, locale_filter) {
    const $selects = $menu.find(".luckysheet-filter-custom-op1, .luckysheet-filter-custom-op2");
    if ($selects.length == 0 || $selects.eq(0).children().length > 0) {
        return;
    }

    const labels = {
        textinclude: locale_filter.conditionCellTextContain,
        textnotinclude: locale_filter.conditionCellTextNotContain,
        textstart: locale_filter.conditionCellTextStart,
        textend: locale_filter.conditionCellTextEnd,
        textequal: locale_filter.conditionCellTextEqual,
        morethan: locale_filter.conditionCellGreater,
        moreequalthan: locale_filter.conditionCellGreaterEqual,
        lessthan: locale_filter.conditionCellLess,
        lessequalthan: locale_filter.conditionCellLessEqual,
        equal: locale_filter.conditionCellEqual,
        noequal: locale_filter.conditionCellNotEqual,
    };

    const html = FILTER_CUSTOM_OPERATORS.map(function (op) {
...
function showFilterConditionInputs(type, value) {
    const $menu = $("#luckysheet-filter-menu");
    $menu.find(".luckysheet-filter-selected-input").hide();

    if (type == "2") {
        $menu.find(".luckysheet-filter-selected-input2").show();
        $menu.find(".luckysheet-filter-selected-andor").show();
        $menu.find(".luckysheet-filter-selected-input2 input").prop("type", "number");
    }
    else if (type == "3") {
        $menu.find(".luckysheet-filter-selected-custom").show();
        $menu.find(".luckysheet-filter-selected-andor").show();
        fillFilterCustomOperators($menu, locale().filter);
    }
    else if (type == "1") {
        $menu.find(".luckysheet-filter-selected-input").eq(0).show();
        if (value == "dateequal" || value == "datelessthan" || value == "datemorethan") {
            $menu.find(".luckysheet-filter-selected-input").eq(0).find("input").prop("type", "date");
        }
        else if (value == "morethan" || value == "moreequalthan" || value == "lessthan" || value == "lessequalthan" || value == "equal" || value == "noequal") {
            $menu.find(".luckysheet-filter-selected-input").eq(0).find("input").prop("type", "number");
        }
        else {
            $menu.find(".luckysheet-filter-selected-input").eq(0).find("input").prop("type", "text");
        }
    }
}

function restoreFilterAndOr($top, value) {
    const saved = $top.data("byconditionand");
    let useAnd = true;
    if (saved === false || saved === "false" || saved === 0) {
        useAnd = false;
    }
    else if (saved === true || saved === "true" || saved === 1) {
        useAnd = true;
    }
    else if (value == "noinclude") {
        useAnd = false;
    }
    $("#luckysheet-filter-menu input[name='luckysheet-filter-andor']").prop("checked", false);
    $("#luckysheet-filter-menu input[name='luckysheet-filter-andor'][value='" + (useAnd ? "and" : "or") + "']").prop("checked", true);
}
...
function initialFilterHandler(){
    ...
    if ($("#luckysheet-filter-submenu .luckysheet-filter-custom-item").length == 0) {
        $("#luckysheet-filter-submenu").append(
            '<div data-value="custom" data-type="3" class="luckysheet-cols-menuitem luckysheet-mousedown-cancel luckysheet-filter-custom-item">' +
                '<div class="luckysheet-cols-menuitem-content luckysheet-mousedown-cancel">' + locale_filter.conditionCellCustom + '</div>' +
            '</div>'
        );
    }
    fillFilterCustomOperators($("#luckysheet-filter-menu"), locale_filter);
    ...
    $("#luckysheet-filter-submenu").mouseover(function () {
        ...
        if ($type == "2") {
            ...
            showFilterConditionInputs("2", $value);
            $("#luckysheet-filter-menu input[name='luckysheet-filter-andor'][value='" + ($value == "noinclude" ? "or" : "and") + "']").prop("checked", true);
        }
        else if ($type == "3") {
            $("#luckysheet-filter-selected span").data("type", "3");
            showFilterConditionInputs("3", $value);
            $("#luckysheet-filter-menu input[name='luckysheet-filter-andor'][value='and']").prop("checked", true);
        }
        else if ($type == "0") {
            $("#luckysheet-filter-selected span").data("type", "0");
            showFilterConditionInputs("0", $value);
        }
        else {
            $("#luckysheet-filter-selected span").data("type", "1");
            showFilterConditionInputs("1", $value);
        }
    ...
    $("#luckysheet-cell-main").on("click", ".luckysheet-filter-options", function (e) {
        ...
        let byconditionvalue = $t.data("byconditionvalue");
        $("#luckysheet-filter-selected span").data("value", byconditionvalue).data("type", byconditiontype).text($t.data("byconditiontext"));
        restoreFilterAndOr($t, byconditionvalue);
        showFilterConditionInputs(byconditiontype, byconditionvalue);
        ...
        else if (byconditiontype == "3") {
            fillFilterCustomOperators($("#luckysheet-filter-menu"), locale_filter);
            $("#luckysheet-filter-menu .luckysheet-filter-custom-op1").val($t.data("byconditionoperator1") || "morethan");
            $("#luckysheet-filter-menu .luckysheet-filter-custom-op2").val($t.data("byconditionoperator2") || "lessthan");
            $("#luckysheet-filter-menu .luckysheet-filter-custom-val1").val($t.data("byconditionvalue1"));
            $("#luckysheet-filter-menu .luckysheet-filter-custom-val2").val($t.data("byconditionvalue2"));
        }
        ...
        setTimeout(function () {
            ...
            for (let r = st_r + 1; r <= ed_r; r++) {
                ...
                if(cell != null && !isRealNull(cell.v) && cell.ct != null && cell.ct.t == "d" ){ //单元格是日期
                    ...
                    if((cell == null || isRealNull(cell.v)) && cell?.mc){
                        const { r, c } = cell.mc;
                        valueCell = Store.flowdata[r][c];
                    }
                    const valueInfo = getFilterValueInfo(valueCell);
                    if(!(valueInfo.key in vmap)){
                        vmap[valueInfo.key] = {
                            text: valueInfo.text,
                            count: 0,
                            isNumber: valueInfo.isNumber,
                            sortValue: valueInfo.sortValue,
                        };
                    }
                    vmap[valueInfo.key].count++;

                    if(r in rowhidden){
                        vmap_uncheck[valueInfo.key] = 0;
                    }
                }
            }
            ...
            if(JSON.stringify(vmap).length > 2){
                const valueInfos = Object.keys(vmap).map(function (key) {
                    return {
                        key: key,
                        text: vmap[key].text,
                        isNumber: vmap[key].isNumber,
                        sortValue: vmap[key].sortValue,
                    };
                });
                const sortedInfos = sortFilterValueKeys(valueInfos);

                for(let i = 0; i < sortedInfos.length; i++){
                    const info = sortedInfos[i];
                    const text = info.key == "null#$$$#null" ? locale_filter.valueBlank : info.text;
                    let dataHtml;
                    if(info.key in vmap_uncheck){
                        dataHtml =  '<div class="textBox luckysheet-mousedown-cancel cf" data-check="false" data-filter="'+ info.key +'" title="'+ text +'">' +
                                            '<input class="luckysheet-mousedown-cancel" type="checkbox"/>' +
                                            '<label class="luckysheet-mousedown-cancel">' + text + '</label>' +
                                            '<span class="luckysheet-mousedown-cancel count">( ' + vmap[info.key].count + ' )</span>' +
                                        '</div>';
                    }
                    else{
                        dataHtml =  '<div class="textBox luckysheet-mousedown-cancel cf" data-check="true" data-filter="'+ info.key +'" title="'+ text +'">' +
                                            '<input class="luckysheet-mousedown-cancel" type="checkbox" checked="checked"/>' +
                                            '<label class="luckysheet-mousedown-cancel">' + text + '</label>' +
                                            '<span class="luckysheet-mousedown-cancel count">( ' + vmap[info.key].count + ' )</span>' +
                                        '</div>';
    ...
    $("#luckysheet-filter-confirm").click(function () {
        ...
        if ($("#luckysheet-filter-bycondition").next().is(":visible") && $("#luckysheet-filter-byvalue").next().is(":hidden") && $("#luckysheet-filter-selected span").data("value") != "null") {
            ...
            if (type == "0") {
                ...
                caljs["and"] = $("#luckysheet-filter-menu input[name='luckysheet-filter-andor']:checked").val() != "or";
            }
            else if (type == "3") {
                caljs["type"] = "3";
                caljs["operator1"] = $("#luckysheet-filter-menu .luckysheet-filter-custom-op1").val();
                caljs["operator2"] = $("#luckysheet-filter-menu .luckysheet-filter-custom-op2").val();
                caljs["value1"] = $("#luckysheet-filter-menu .luckysheet-filter-custom-val1").val();
                caljs["value2"] = $("#luckysheet-filter-menu .luckysheet-filter-custom-val2").val();
                caljs["and"] = $("#luckysheet-filter-menu input[name='luckysheet-filter-andor']:checked").val() != "or";
            }
            else {
                caljs["type"] = "1";
            ...
            for (let r = st_r + 1; r <= ed_r; r++) {
                ...
                let cell = Store.flowdata[r][cindex];
                if (shouldHideByCaljs(cell, caljs, dateSerialFromFilterInput)) {
                    rowhidden[r] = 0;
                }
            }
        }
            ...
            for (let r = st_r + 1; r <= ed_r; r++) {
                ...
                if((cell == null || isRealNull(cell.v)) && cell?.mc){
                    ...
                    value = getFilterValueInfo(mainCell).key;
                }
                    ...
                    value = getFilterValueInfo(cell).key;
                }

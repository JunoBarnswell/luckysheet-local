...
const PASTE_STYLE_KEYS = ["bg", "ff", "fc", "bl", "it", "fs", "cl", "un", "ht", "vt", "tb", "tr", "rt", "ct"];

function pickPasteValueOnly(cell) {
    if (cell == null) {
        return null;
    }
...
function mergePasteFormatOnly(dest, source) {
    const next = dest == null || typeof dest !== "object" ? {} : $.extend(true, {}, dest);
    if (source == null || typeof source !== "object") {
        return next;
    }
    for (let i = 0; i < PASTE_STYLE_KEYS.length; i++) {
        const key = PASTE_STYLE_KEYS[i];
        if (source[key] != null) {
            next[key] = typeof source[key] === "object" ? $.extend(true, {}, source[key]) : source[key];
        }
    }
    return next;
}
...
function applyPasteMode(dest, source, mode) {
    ...
    if (mode == "format") {
        return mergePasteFormatOnly(dest, source);
    }
    return source == null ? null : $.extend(true, {}, source);
}

function parseExternalTdValue($td, text) {
    const cell = {};
    const raw = $td.attr("data-sheets-value");
    if (raw) {
        try {
            const obj = JSON.parse(raw);
            const kind = obj[1] != null ? obj[1] : obj["1"];
            if (kind === 3 || kind === "3") {
                cell.v = obj[3] != null ? obj[3] : obj["3"];
                cell.ct = { fa: "General", t: "n" };
                cell.m = String(cell.v);
                return cell;
            }
            if (kind === 4 || kind === "4") {
                cell.v = !!(obj[4] != null ? obj[4] : obj["4"]);
                cell.ct = { fa: "General", t: "b" };
                cell.m = cell.v ? "TRUE" : "FALSE";
                return cell;
            }
            const textVal = obj[2] != null ? obj[2] : obj["2"];
            if (textVal != null) {
                const mask = genarate(textVal);
                cell.v = mask[2];
                cell.ct = mask[1];
                cell.m = mask[0];
                return cell;
    ...
    const xnum = $td.attr("x:num") || $td.attr("ss:num");
    if (xnum != null && xnum !== "" && isRealNum(xnum)) {
        cell.v = parseFloat(xnum);
        cell.ct = { fa: "General", t: "n" };
        cell.m = String(cell.v);
        return cell;
    }

    const mso = $td.attr("data-sheets-numberformat") || $td.css("mso-number-format");
    const mask = genarate(text);
    cell.v = mask[2];
    cell.ct = mask[1];
    cell.m = mask[0];
    if (mso && typeof mso === "string" && mso !== "none") {
        cell.ct = cell.ct || {};
        cell.ct.fa = mso.replace(/^['"]|['"]$/g, "");
    }
    return cell;
}
...
const selection = {
    ...
    parseExternalTdValue: parseExternalTdValue,
    applyPasteMode: applyPasteMode,
    pasteSpecial: function(mode) {
        const _locale = locale();
        const locale_paste = _locale.paste;
        if (!Store.luckysheet_copy_save || Store.luckysheet_copy_save.copyRange == null || Store.luckysheet_copy_save.copyRange.length == 0) {
            if (isEditMode()) {
                alert(locale_paste.noCopyRange || locale_paste.errorNotAllowMulti);
            } else {
                tooltip.info(locale_paste.noCopyRange || locale_paste.warning, "");
            }
            return;
        }
        if (Store.luckysheet_paste_iscut) {
            Store.luckysheet_paste_iscut = false;
            this.pasteHandlerOfCutPaste(Store.luckysheet_copy_save);
            this.clearcopy();
            return;
        }
        this.pasteHandlerOfCopyPaste(Store.luckysheet_copy_save, mode);
    },
    pasteHandler: function(data, borderInfo, mode) {
        ...
        if (typeof data == "object") {
            ...
            for (let h = minh; h <= maxh; h++) {
                ...
                for (let c = minc; c <= maxc; c++) {
                    if (mode != "value" && mode != "format" && getObjType(x[c]) == "object" && "mc" in x[c]) {
                        if ("rs" in x[c].mc) {
                    ...
                    x[c] = applyPasteMode(x[c], value, mode);

                    if (mode != "format" && value != null && x[c] != null && "mc" in x[c]) {
                        if (x[c]["mc"].rs != null) {
                            x[c]["mc"].r = h;
                            x[c]["mc"].c = c;
                    ...
                    if (mode != "value" && borderInfo && borderInfo[h - minh + "_" + (c - minc)]) {
                        let bd_obj = {
                            rangeType: "cell",
                            value: {
                                row_index: h,
            ...
            for (let r = 0; r < rlen; r++) {
                ...
                for (let c = 0; c < clen; c++) {
                    ...
                    if (originCell instanceof Object) {
                        sourceCell = $.extend(true, {}, originCell);
                        sourceCell.v = value;
                        if (sourceCell.ct != null && sourceCell.ct.fa != null) {
                            sourceCell.m = update(sourceCell["ct"]["fa"], value);
                        } else {
                            sourceCell.m = value;
                        }

                        if (mode != "format" && sourceCell.f != null && sourceCell.f.length > 0) {
                            sourceCell.f = "";
                            formula.delFunctionGroup(r + curR, c + curC, Store.currentSheetIndex);
                        }
                    } else {
                        let mask = genarate(value);
                        sourceCell = { v: mask[2], ct: mask[1], m: mask[0] };
                    }
                    x[c + curC] = applyPasteMode(originCell, sourceCell, mode);
                }
                d[r + curR] = x;
            }
    ...
    pasteHandlerOfCopyPaste: function(copyRange, mode) {
        if (!checkProtectionLockedRangeList(Store.luckysheet_select_save, Store.currentSheetIndex)) {
            return;
        }
            ...
            for (let tc = 1; tc <= timesC; tc++) {
                ...
                for (let h = mth; h < maxrowCache; h++) {
                    ...
                    for (let c = mtc; c < maxcellCahe; c++) {
                        if (mode != "value" && borderInfoCompute[c_r1 + h - mth + "_" + (c_c1 + c - mtc)]) {
                            let bd_obj = {
                                rangeType: "cell",
                                value: {
                                    row_index: h,
                            ...
                            cfg["borderInfo"].push(bd_obj);
                        } else if (mode != "value" && borderInfoCompute[h + "_" + c]) {
                            let bd_obj = {
                                rangeType: "cell",
                                value: {
                                    row_index: h,
                        ...
                        //数据验证 复制（仅全部粘贴；仅值/仅格式不改验证）
                        if (mode != "value" && mode != "format" && c_dataVerification[c_r1 + h - mth + "_" + (c_c1 + c - mtc)]) {
                        ...
                        if (mode != "format" && mode != "value" && value != null && value.f != null) {
                            let func = value.f;
                        ...
                        if (mode != "format" && value != null && copyHasMC && x[c] != null && "mc" in x[c]) {
                            if (x[c]["mc"].rs != null) {
                                x[c]["mc"].r = h;
                                x[c]["mc"].c = c;

...
import { genarate, update, is_date, isFormatCoverable, isScientificNotationCell, isAllowedPatternForScientificNotation } from "../global/format";
        ...
        if (attr == "ct") {
            for (let r = row_st; r <= row_ed; r++) {
                ...
                for (let c = col_st; c <= col_ed; c++) {
                    ...
                    // #7383 空格式 coverable:false，不得覆盖已有 ct
                    if (!isFormatCoverable(foucsStatus)) {
                        continue;
                    }

                    // #7163 科学计数显示格只允许 General / 文本 / scientific
                    if (isScientificNotationCell(cell) && !isAllowedPatternForScientificNotation(foucsStatus)) {
                        continue;
                    }

...
import { orderbydata,  sortColumnSeletion, collectVisibleSortRows, writeVisibleSortRows, abortSortIfFormulaIntersect } from '../global/sort';
...
/**
 * 主排序入口：多关键字 + 跳过隐藏行 + 数组公式相交检测。
 * 筛选下拉菜单内排序（filter.js orderbydatafiler）仍为单列，见
 * docs/sort-filter-menu-limit.md 与 locale.sort.filterMenuSingleColumnLimit。
 */
    ...
    $("#luckysheetorderby").click(function () {
        ...
        if (luckysheet_sort_initial) {
            ...
            $("#luckysheet-sort-modal-confirm").click(function () {
                ...
                // 数组公式 / 动态数组 spill 与排序区相交则中止（对照 UV sheets-sort.service formulaCheck）
                if(abortSortIfFormulaIntersect(str, r2, c1, c2)){
                    return;
                }

                // 跳过 config.rowhidden 与手动隐藏行，只在可见行之间重排
                let collected = collectVisibleSortRows(d, str, r2, c1, c2, Store.config);

...
const sheetmanage = {
    ...
    buildGridData: function(file) {
        ...
        let data;
        if (file.data && isSparseGrid(file.data)) {
            data = file.data;
            ensureSparseSize(data, row - 1, column - 1);
        }
        else if (file.data && file.data.length > 0) {
            data = asSparseGrid(file.data, row, column);
            const occupied = sparseGridToCelldata(data);
            for (let i = 0; i < occupied.length; i++) {
                setcellvalue(occupied[i].r, occupied[i].c, data, occupied[i].v);
            }
        }
        else {
            data = createSparseGrid(row, column);
            if (celldata && celldata.length > 0) {
                for (let i = 0; i < celldata.length; i++) {
                    ...
                    ensureSparseSize(data, item.r, item.c);
    ...
    addGridData: function(celldata, row, column) {
        let data = createSparseGrid(row, column);
    ...
    storeSheetParam: function(persistScroll) {
        ...
        if (persistScroll !== false) {
            let scrollbarX = $("#luckysheet-scrollbar-x")[0];
            let scrollbarY = $("#luckysheet-scrollbar-y")[0];
            if (scrollbarX != null && scrollbarX.scrollWidth > scrollbarX.offsetWidth) {
                file["scrollLeft"] = $("#luckysheet-scrollbar-x").scrollLeft();
            }

            if (scrollbarY != null && scrollbarY.scrollHeight > scrollbarY.offsetHeight) {
    ...
    applySheetScroll: function(file) {
        if (file == null) {
            let index = this.getSheetIndex(Store.currentSheetIndex);
            file = Store.luckysheetfile[index];
        }
        if (file == null) {
            return;
        }

        let zoomRatio = Store.zoomRatio == null ? 1 : Store.zoomRatio;
        let scrollLeft = file["scrollLeft"] != null && file["scrollLeft"] > 0 ? file["scrollLeft"] * zoomRatio : 0;
        let scrollTop = file["scrollTop"] != null && file["scrollTop"] > 0 ? file["scrollTop"] * zoomRatio : 0;

        if ($("#luckysheet-scrollbar-x").length) {
            $("#luckysheet-scrollbar-x").scrollLeft(scrollLeft);
        }
        if ($("#luckysheet-scrollbar-y").length) {

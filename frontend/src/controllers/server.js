...
const server = createContextualModule('server', {
    ...
    saveParam: function (type, index, value, params) {
		...
		// 切 sheet：发给后台落库激活状态；其他客户端收到 shs 不得切自己的视图（非 OT）
		d.ver = ++_this.collabSeq;
		d.ts = Date.now();
		if(type == "v" && params){
			_this.cellClock[_this.cellClockKey(index, params.r, params.c)] = d.ts;
		}
		else if(type == "rv" && params && params.range){
			let rr = params.range.row, cc = params.range.column;
			for(let r = rr[0]; r <= rr[1]; r++){
				for(let c = cc[0]; c <= cc[1]; c++){
					_this.cellClock[_this.cellClockKey(index, r, c)] = d.ts;
				}
			}
		}
    ...
    sendSocket: function(msg){
        ...
        _this.outboundQueue.push(msg);
        if(_this.outboundQueue.length > _this.outboundQueueLimit){
            _this.outboundQueue.shift();
        }
        return false;
    },
    flushOutboundQueue: function(){
        let _this = this;
        if(_this.websocket == null || _this.websocket.readyState !== 1){
            return;
        }
        while(_this.outboundQueue.length > 0){
            _this.websocket.send(_this.outboundQueue.shift());
        }
    },
    closeWebSocket: function(intentional){
        let _this = this;
        _this.intentionalClose = !!intentional;
        if(_this.retryTimer){
        ...
        if(_this.websocket != null){
            try{
                ...
                if(_this.websocket.readyState === 0 || _this.websocket.readyState === 1){
                    _this.websocket.close(1000, "client close");
                }
            }catch(e){}
    ...
    scheduleReconnect: function(){
        ...
        if(_this.reconnectAttempt >= _this.maxReconnectAttempts){
            showloading(locale().websocket.reconnectFailed || locale().websocket.refresh);
            return;
        }
    ...
    cellClockKey: function(index, r, c){
        return String(index) + "_" + r + "_" + c;
    },
    shouldApplyRemoteCell: function(index, r, c, ts){
        if(ts == null){
            return true;
        }
        let key = this.cellClockKey(index, r, c);
        let prev = this.cellClock[key];
        if(prev != null && Number(ts) < Number(prev)){
            return false;
        }
        this.cellClock[key] = Number(ts);
        return true;
    },
    openWebSocket: function(){
        ...
        if('WebSocket' in window){
			...
			if(_this.websocket != null && (_this.websocket.readyState === 0 || _this.websocket.readyState === 1)){
				return;
			}
			if(_this.retryTimer){
				clearInterval(_this.retryTimer);
        ...
        method.createHookFunction('cooperativeMessage', data)
                ...
                let ackPayload = data.data;
                if(typeof ackPayload === "string" && ackPayload){
                    try { ackPayload = JSON.parse(ackPayload); } catch(e) { ackPayload = null; }
                }
                if(ackPayload && ackPayload.ver != null){
                    _this.lastAckVer = Math.max(_this.lastAckVer, Number(ackPayload.ver) || 0);
                }
                if(data.status === "1" || data.returnMessage === "error"){
                    console.info("collab ack failed", data);
                    return;
                }
                if(ackPayload && ackPayload.v && ackPayload.v.index != null){
            ...
            _this.websocket.onclose = runInServerInstance(instanceId, function(e){
				...
				_this.websocket = null;
				if(_this.intentionalClose || e.code === 1000){
					return;
				}
    ...
    wsUpdateMsg: function(item) {
		...
		if(item.ver != null){
			this.lastRemoteVer = Math.max(this.lastRemoteVer, Number(item.ver) || 0);
		}

		// 其他人激活 sheet 只入库，不切换本机当前视图
		if(type == "shs"){
			return;
		}
	    ...
	    if(type == "v"){ //单个单元格数据更新
			...
			if(!this.shouldApplyRemoteCell(index, r, c, item.ts)){
				return;
			}
	        file.data[r][c] = value;
	        ...
	        let rc = item.rc,
	        	...
	        	mc = value.mc,
	        	borderInfo = value.borderInfo;
	        let data = cloneSheetData(file.data);

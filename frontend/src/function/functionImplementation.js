...
const functionImplementation = {
    ...
    "SUBTOTAL": function() {
        ...
        try {
            function getSubtotalRowhidden(ref) {
                var files = getluckysheetfile();
                var file = null;
                if (ref != null && ref.sheetName != null) {
                    for (var si = 0; si < files.length; si++) {
                        if (files[si].name == ref.sheetName) {
                            file = files[si];
                            break;
                        }
                    }
                ...
                if (file == null) {
                    var sheetIndex = Store.calculateSheetIndex != null ? Store.calculateSheetIndex : Store.currentSheetIndex;
                    file = files[getSheetIndex(sheetIndex)];
                }
                if (file != null && file.config != null && file.config.rowhidden != null) {
                    return file.config.rowhidden;
                }
                if (Store.config != null && Store.config.rowhidden != null) {
                    return Store.config.rowhidden;
                }
                return {};
            }

            function filterSubtotalHiddenRows(refs) {
                var filtered = [];
                for (var ri = 0; ri < refs.length; ri++) {
                    var ref = refs[ri];
                    if (getObjType(ref) != "object" || ref.startCell == null) {
                        filtered.push(ref);
                        continue;
                    }

                    var rowhidden = getSubtotalRowhidden(ref);
                    if (rowhidden == null || Object.keys(rowhidden).length == 0) {
                        filtered.push(ref);
                        continue;
                    }

                    var cellrange = formula.getcellrange(ref.startCell);
                    if (cellrange == null || cellrange.row == null) {
                        filtered.push(ref);
                        continue;
                    }

                    var startRow = cellrange.row[0];
                    if (ref.rowl == 1 && ref.coll == 1) {
                        if (rowhidden[startRow] != null) {
                            filtered.push({
                    ...
                    if (ref.data == null || getObjType(ref.data) != "array") {
                        filtered.push(ref);
                        continue;
                    }

                    var newData = [];
                    for (var rowi = 0; rowi < ref.data.length; rowi++) {
                        if (rowhidden[startRow + rowi] == null) {
                            newData.push(ref.data[rowi]);
                        }
                    }
            ...
            arr.shift();
            arr = filterSubtotalHiddenRows(arr);

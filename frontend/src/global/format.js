...
var make_ssf = function make_ssf(SSF) {
    ...
    */
    ...
    function format(fmt, v, o) {
        ...
        //new runze 增加万 亿 格式（LS 扩展，见 docs/numfmt-wan-extension.md）
        //注："w":2万2500  "w0":2万2500  "w0.0":2万2500.2  "w0.00":2万2500.23......自定义精确度
        // 与 Excel fa / Univer numfmt 不兼容，禁止改写成 scientific 或 #,##0
        var reg = /^(w|W)((0?)|(0\.0+))$/;
        if(!!sfmt.match(reg)){
            if(isNaN(v)){
                return v;
...
export function genarate(value) {//万 单位格式增加！！！
    ...
    else if(isScientificNotationValue(value) || (isRealNum(value) && Math.abs(parseFloat(value)) > 0 && (Math.abs(parseFloat(value)) >= 1e+11 || Math.abs(parseFloat(value)) < 1e-9))){
        // #7163 科学计数法边界：已是科学计数显示的值只允许 default/text/scientific，避免 JS 精度展开
        if (value.length > 16) {
            v = value;
            ct = { 'fa': '@', 't': 's' }
            ...
            ct = { "fa": getScientificNotationFormatFromValue(value), "t": "n" };
            m = SSF.format(ct.fa, v);
        }
    }
...
export function isScientificNotationCell(cell) {
    if (cell == null || typeof cell !== "object") {
        return isScientificNotationValue(cell);
    }
    const fa = cell.ct && cell.ct.fa;
    if (fa && /e[+-]/i.test(String(fa))) {
        return true;
    }
    return isScientificNotationValue(cell.v);
}

export function getScientificNotationFormatFromValue(value) {
    const text = String(value);
    const eIndex = text.search(/e/i);
    const decimalIndex = text.indexOf(".");
    const decimalLength = eIndex > -1 && decimalIndex > -1 ? eIndex - decimalIndex - 1 : 0;
    return "0." + new Array(Math.max(2, Math.min(decimalLength, 5)) + 1).join("0") + "E+00";
}

export function isAllowedPatternForScientificNotation(pattern) {
    if (pattern == null || pattern === "" || pattern === "General" || pattern === 0) {
        return true;
    }
    if (pattern === "@" || pattern === 49) {
        return true;
    }
    return /e[+-]/i.test(String(pattern));
}

/**
 * #7383 空格式不可覆盖已有 ct.fa（coverable:false）
 */
export function isFormatCoverable(fa) {
    if (fa == null) {
        return false;
    }

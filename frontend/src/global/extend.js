...
import { syncFilterOnInsert, syncFilterOnDelete } from "../controllers/filterSync";
...
function luckysheetextendtable(type, index, value, direction, sheetIndex) {
    ...
    //筛选配置变动
    let newFilterObj = syncFilterOnInsert(file.filter_select, file.filter, type, index, value, direction);
...
function luckysheetdeletetable(type, st, ed, sheetIndex) {
    ...
    //筛选配置变动
    let newFilterObj = syncFilterOnDelete(file.filter_select, file.filter, type, st, ed);

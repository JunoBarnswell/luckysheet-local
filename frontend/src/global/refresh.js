import rhchInit, { patchVisibleAxisSlice } from './rhchInit';
...
function runExecFunction(range, index, data){
    ...
    let dirtyCells = drainFormulaDirtyCells();
    for (let i = 0; i < dirtyCells.length; i++) {
        formula.execFunctionExist.push(dirtyCells[i]);
    }
...
function resolveRefreshDirtyRect(range, dirtyRect) {
    if (dirtyRect == null) {
        dirtyRect = unionRangesToDirtyRect(range);
    }
    dirtyRect = expandDirtyRectForMerges(dirtyRect, Store.config && Store.config.merge);
    if (dirtyRect != null && isSparseGrid(Store.flowdata)) {
        growSparseToRect(Store.flowdata, dirtyRect);
    }
    return dirtyRect;
}

function jfrefreshgrid(data, range, allParam, isRunExecFunction = true, isRefreshCanvas = true, dirtyRect) {
    if(data == null){
        data = Store.flowdata;
    }
...
function luckysheetrefreshgrid(scrollWidth, scrollHeight, dirtyRect) {
    ...
    let currentView = visibleRectFromSearch(
        Store.visibledatarow,
    ...
    if (dirtyRect == null) {
        // 滚动路径不做“只清新条带”：没有 canvas bitblt 时局部 clear 会留下错位旧像素。
        // #7394 的 merge-aware 增量用在写入 dirtyRect（jfrefreshgrid / setRangeValue）。
        dirtyRect = null;
    }
    else {
        dirtyRect = expandDirtyRectForMerges(normalizeDirtyRect(dirtyRect), Store.config && Store.config.merge);
        if (dirtyRect != null && dirtyRect.axisPatch !== false && !dirtyRect.fromScroll) {
            patchVisibleAxisSlice(dirtyRect);
        }
    }
    ...
    Store.drawViewportRect = currentView;
    Store.drawDirtyRect = dirtyRect;
    ...
    if (luckysheetFreezen.freezenverticaldata != null || luckysheetFreezen.freezenhorizontaldata != null) {
        ...
        luckysheetDrawMain(scrollWidth, scrollHeight, null, null, null, null, null, null, null, dirtyRect);

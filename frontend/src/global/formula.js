import { replaceHtml, getObjType, chatatABC, ABCatNum, luckysheetfontformat, focusEditor } from "../utils/util";
...
const luckysheetformula = createContextualModule("formula", {
    ...
    setCaretPosition: function(textDom, children, pos) {
        try {
            ...
            sel.removeAllRanges();
            sel.addRange(range);
            focusEditor(el, { preventScroll: true });
        } catch (err) {
    ...
    _imeInputTimer: null,
    _imeEndTimer: null,
    bindIMEComposition: function() {
        if (this._imeBound) {
            return;
        }
        this._imeBound = true;
        const _this = this;
        const selector = "#luckysheet-rich-text-editor, #luckysheet-functionbox-cell";
        $(document).on("compositionstart.luckysheetIME", selector, function() {
            _this.isComposing = true;
            _this.compositionJustEnded = false;
            if (_this._imeInputTimer) {
                clearTimeout(_this._imeInputTimer);
                _this._imeInputTimer = null;
            }
        });
        $(document).on("compositionupdate.luckysheetIME", selector, function() {
            _this.isComposing = true;
        });
        $(document).on("compositionend.luckysheetIME", selector, function() {
            _this.isComposing = false;
            _this.compositionJustEnded = true;
            if (_this._imeEndTimer) {
                clearTimeout(_this._imeEndTimer);
            }
            _this._imeEndTimer = setTimeout(function() {
                _this.compositionJustEnded = false;
                const $editor = $("#luckysheet-rich-text-editor");
                const $box = $("#luckysheet-functionbox-cell");
                if ($editor.is(":focus")) {
                    _this.functionInputHanddler($box, $editor, 0);
                } else if ($box.is(":focus")) {
                    _this.functionInputHanddler($editor, $box, 0);
                }
            }, 0);
        });
    },
    shouldSkipIMECommit: function(event) {
        if (this.isComposing || this.compositionJustEnded) {
            return true;
        }
        if (event != null && (event.isComposing || event.keyCode === 229)) {
            return true;
        }
        return false;
    },
    functionInputHanddler: function($to, $input, kcode) {
        ...
        let _this = this;
        _this.bindIMEComposition();

        if (_this.isComposing || kcode === 229) {
            if (_this._imeInputTimer) {
                clearTimeout(_this._imeInputTimer);
                _this._imeInputTimer = null;
            }
        ...
        _this._imeInputTimer = setTimeout(function() {
    ...
    execFunctionGroup: function(origin_r, origin_c, value, index, data, isForce = false) {
        ...
        Object.keys(formulaObjects).forEach((key) => {
            ...
            if (isForce || key in updateValueOjects) {
                updateValueArray.push(formulaObject);
            }
        });

...
function luckysheetDrawMain(
    ...
    let renderDirty = normalizeDirtyRect(dirtyRect != null ? dirtyRect : Store.drawDirtyRect);
    if (renderDirty != null) {
        renderDirty = expandDirtyRectForMerges(renderDirty, Store.config && Store.config.merge);
        let visibleDirty = intersectDirtyRect(renderDirty, {
            row: [dataset_row_st, dataset_row_ed],
            column: [dataset_col_st, dataset_col_ed],
        });
        if (visibleDirty == null) {
            luckysheetTableContent.restore();
            return;
        }
        dataset_row_st = visibleDirty.row[0];
        dataset_row_ed = visibleDirty.row[1];
        dataset_col_st = visibleDirty.column[0];
        dataset_col_ed = visibleDirty.column[1];
    }
    ...
    if (renderDirty != null) {
        let clearX = offsetLeft + fill_col_st - scrollWidth - 1;
        let clearY = offsetTop + fill_row_st - scrollHeight - 1;
        let clearW = fill_col_ed - fill_col_st + 2;
        let clearH = fill_row_ed - fill_row_st + 2;
        luckysheetTableContent.clearRect(clearX, clearY, clearW, clearH);
        luckysheetTableContent.fillStyle = "#ffffff";
        luckysheetTableContent.fillRect(clearX, clearY, clearW, clearH);
    } else {
    ...
    for (let r = dataset_row_st; r <= dataset_row_ed; r++) {
        ...
        for (let c = dataset_col_st; c <= dataset_col_ed; c++) {
            ...
            //数据透视表：占位框用 pivotTableBoundary，禁止写死 12×6
            let pivotBoundary = pivotTable.pivotTableBoundary;
            let pivotRows = pivotBoundary && pivotBoundary[0] != null ? pivotBoundary[0] : 12;
            let pivotCols = pivotBoundary && pivotBoundary[1] != null ? pivotBoundary[1] : 6;
            if (pivotRows < 1) {
                pivotRows = 12;
            }
            if (pivotCols < 1) {
            ...
            let pivotLastRow = pivotRows - 1;
            let pivotLastCol = pivotCols - 1;
            let pivotTitleRow = Math.floor(pivotRows / 2);
            let pivotTitleCol = Math.floor(pivotCols / 2);

            if (!!Store.luckysheetcurrentisPivotTable && pivotTable.drawPivotTable) {
                if ((c == 0 || c == pivotLastCol) && r <= pivotLastRow) {
                    luckysheetTableContent.beginPath();
                ...
                if ((r == 2 || r == pivotLastRow) && c <= pivotLastCol) {
                    luckysheetTableContent.beginPath();
                ...
                if (r == pivotTitleRow && c == pivotTitleCol) {
                    luckysheetTableContent.save();
...
let cellRender = function(
    ...
    // Note：黄/橙三角，与 ps 红三角并存；有批注时略向内
    if (noteCtrl.hasNote(r, c)) {
        let inset = cell && cell.ps != null ? 8 * Store.zoomRatio : 0;
        let ns_w = 8 * Store.zoomRatio,
            ns_h = 8 * Store.zoomRatio;

        luckysheetTableContent.beginPath();
        luckysheetTableContent.moveTo(end_c + offsetLeft - ns_w - inset, start_r + offsetTop);
        luckysheetTableContent.lineTo(end_c + offsetLeft - inset, start_r + offsetTop);
        luckysheetTableContent.lineTo(end_c + offsetLeft - inset, start_r + offsetTop + ns_h);
        luckysheetTableContent.fillStyle = "#F5A623";
        luckysheetTableContent.fill();
        luckysheetTableContent.closePath();
    }

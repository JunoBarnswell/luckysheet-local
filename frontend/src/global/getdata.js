...
import { asSparseGrid, isSparseGrid, ensureSparseSize } from './sparseGrid';
...
export function datagridgrowth(data, addr, addc, iscallback) {
    ...
    if (data != null && data.length === 1 && isSparseGrid(data[0]) && !isSparseGrid(data)) {
        data = data[0];
    }

    let grid = asSparseGrid(data);
    let nextRow = grid.length + addr;
    let nextCol = (grid[0] != null ? grid[0].length : 0) + addc;
    ensureSparseSize(grid, nextRow - 1, nextCol - 1);

    if(!!iscallback){
        server.saveParam("all", Store.currentSheetIndex, grid.length, { "k": "row" });
        server.saveParam("all", Store.currentSheetIndex, grid[0] != null ? grid[0].length : 0, { "k": "column" });
    }

...
export function patchVisibleAxisSlice(dirtyRect) {
    if (dirtyRect == null || dirtyRect.row == null || dirtyRect.column == null) {
        return;
    }
    ...
    let rowCount = Store.flowdata != null ? Store.flowdata.length : Store.visibledatarow.length;
    let colCount = Store.flowdata != null && Store.flowdata[0] != null ? Store.flowdata[0].length : Store.visibledatacolumn.length;
    let r1 = Math.max(0, dirtyRect.row[0]);
    let c1 = Math.max(0, dirtyRect.column[0]);

    if (rowCount > 0) {
        let base = r1 === 0 ? 0 : Store.visibledatarow[r1 - 1];
        Store.rh_height = base;
        for (let r = r1; r < rowCount; r++) {
            let rowlen = Store.defaultrowlen;
            ...
            if (Store.config["rowhidden"] != null && Store.config["rowhidden"][r] != null) {
                Store.visibledatarow[r] = Store.rh_height;
                continue;
            }
        ...
        Store.visibledatarow.length = rowCount;
    ...
    if (colCount > 0 && Store.visibledatacolumn != null && Store.visibledatacolumn.length > 0) {
        let base = c1 === 0 ? 0 : Store.visibledatacolumn[c1 - 1];
        Store.ch_width = base;
        for (let c = c1; c < colCount; c++) {
            let firstcolumnlen = Store.defaultcollen;
            ...
            if(Store.config["colhidden"] != null && Store.config["colhidden"][c] != null){
                Store.visibledatacolumn[c] = Store.ch_width;
                continue;
            }

            if (firstcolumnlen === 'auto') {
                firstcolumnlen = computeColWidthByContent(Store.flowdata, c, rowCount);
            }
            Store.ch_width += Math.round((firstcolumnlen + 1)*Store.zoomRatio);
            Store.visibledatacolumn[c] = Store.ch_width;
        }
        Store.visibledatacolumn.length = colCount;
        Store.ch_width += 120;
    }
}

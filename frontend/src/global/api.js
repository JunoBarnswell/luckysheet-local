...
import { setAccuracy, setcellvalue, isFormulaString } from "./setdata";
import { orderbydata } from "./sort";
import { rowlenByRange } from "./getRowlen";
import { getdatabyselection, getcellvalue } from "./getdata";
import { luckysheetrefreshgrid, jfrefreshgrid, jfrefreshgrid_rhcw, markFormulaDirty } from "./refresh";
...
export function setCellValue(row, column, value, options = {}) {
    ...
    if(value == null || value.toString().length == 0){
        ...
        if(isFormulaString(value.f)){
            if(isRefresh && file.index == Store.currentSheetIndex){
                let updated = luckysheetformula.updatecell(row, column, value, false);
                if(updated != null && updated.data != null){
                    data = updated.data;
                }
            }
            else{
                let formulaCell = data[row][column] || {};
                formulaCell.f = value.f;
                if(value.ct != null){
                    formulaCell.ct = value.ct;
                }
                delete formulaCell.v;
                delete formulaCell.m;
                data[row][column] = formulaCell;
                formula.insertUpdateFunctionGroup(row, column, file.index);
            }
            markFormulaDirty(row, column, file.index);
        }
        else{
            ...
            else if(data[row][column] != null && data[row][column].v != null){
                curv.v = data[row][column].v;
            }
            if(value.m != null){
                curv.m = value.m;
            }
            if(value.v != null || value.m != null || curv.v != null){
                setcellvalue(row, column, data, curv);
            }
        }
        ...
        for(let attr in value){
            if(attr == "f" || attr == "v" || attr == "m" || attr == "ct"){
                continue;
            }
            let v = value[attr];
    ...
    if(file.index == Store.currentSheetIndex && isRefresh){
        jfrefreshgrid(data, [{ "row": [row, row], "column": [column, column] }], undefined, true);
    }
    else{
        file.data = data;
    }
...
export function setRangeValue(data, options = {}) {
    ...
    for (let r = range.row[0]; r <= range.row[1]; r++) {
        ...
        for (let c = range.column[0]; c <= range.column[1]; c++) {
            let cell = file.data[r][c];
            if(cell != null && isFormulaString(cell.f)){
                formula.insertUpdateFunctionGroup(r, c, file.index);
                markFormulaDirty(r, c, file.index);
                hasFormula = true;
            }
        }
    }
    ...
    if(hasFormula){
        formula.execFunctionGroup(null, null, null, file.index, file.data, true);
    }

    if(file.index == Store.currentSheetIndex){
        jfrefreshgrid(file.data, [{
            row: range.row,
            column: range.column,
    ...
    if(isRefresh) {
        luckysheetrefreshgrid(null, null, {
            row: range.row,
            column: range.column,
        });
    }
...
export function setRangeSort(type, options = {}) {
    ...
    let file = Store.luckysheetfile[order],
        cfg = $.extend(true, {}, file.config),
        fileData = cloneSheetData(file.data);

    if(fileData.length == 0){
        fileData = cloneSheetData(sheetmanage.buildGridData(file));
    }
...
export function clearRange(options = {}) {
    ...
    let d = cloneSheetData(file.data);

    if(d.length == 0){
        d = cloneSheetData(sheetmanage.buildGridData(file));
    }
...
export function getAllSheets() {
    return Store.luckysheetfile.map((file) => {
        let item = {};
        for (let key in file) {
            if (key === "data" || key === "load" || key === "freezen") {
                continue;
            }
            item[key] = file[key];
        }
        if (file.data != null) {
            item.celldata = sheetmanage.getGridData(file.data);
        }
        try {
...
export function toJson(){
    ...
    getluckysheetfile().forEach((file,index)=>{
        ...
        toJsonOptions.data[index].column = file.data[0] != null ? file.data[0].length : 0;

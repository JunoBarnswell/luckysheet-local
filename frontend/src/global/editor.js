...
import { cloneSheetData, isSparseGrid } from './sparseGrid';
...
const editor = {
    ...
    deepCopyFlowData:function(flowData){
        ...
        if(isSparseGrid(flowData)){
            if(_this.deepCopyFlowDataState && isSparseGrid(_this.deepCopyFlowDataCache)){
            ...
            return cloneSheetData(flowData);
        }
    ...
    webWorkerFlowDataCache:function(flowData){
        ...
        if(isSparseGrid(flowData)){
            ...
            _this.deepCopyFlowDataCache = cloneSheetData(flowData);
    ...
    controlHandler: function (dataChe, range) {
        ...
        if(addr > 0 || addc > 0){
            d = datagridgrowth(d, addr, addc, true);
        }
    ...
    clearRangeByindex: function (st_r, ed_r, st_c, ed_c, sheetIndex) {
        ...
        let d = cloneSheetData(Store.luckysheetfile[index]["data"]);

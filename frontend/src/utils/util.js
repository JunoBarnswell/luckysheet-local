...
function focusEditor(element, options) {
    if (element == null) {
        return;
    }

    let opts = options == null ? {} : options;
    let select = opts.select === true;
    let preventScroll = opts.preventScroll !== false;
    let el = element.jquery ? element[0] : element;
    ...
    if (window.getSelection && el.childNodes && el.childNodes.length) {
...
function luckysheetactiveCell() {
    if (!!Store.fullscreenmode) {
        setTimeout(function() {
            focusEditor(document.getElementById("luckysheet-rich-text-editor"), { select: true, preventScroll: true });
        }, 50);
    }
}

...
export type AcceptanceDomain =
  ...
  | 'responsive-shell'
  | 'collaboration';
export type AcceptancePersistence = 'not-applicable' | 'server-roundtrip' | 'xlsx-roundtrip';
...
export const CORE_INTERACTION_CASES: readonly CoreInteractionAcceptanceCase[] = [
  coreInteraction('CORE-001', 'Cloud workbook shell exposes the fixed 1280x720 geometry and real palette entry', 'designer-shell', 'e2e/spreadsheet.spec.ts', ['interaction', 'browser', 'visual'], ['shell geometry', 'command palette entry']),
  ...
  coreInteraction('CORE-006', 'clipboard copy and paste round-trip', 'clipboard', 'e2e/spreadsheet.spec.ts', ['interaction', 'mutation', 'browser', 'persistence'], ['clipboard payload', 'paste result'], 'server-roundtrip'),
  ...
  coreInteraction('CORE-009', 'adds and activates a new editable worksheet from the sheet tab control', 'worksheet', 'e2e/spreadsheet.spec.ts', ['interaction', 'mutation', 'browser', 'persistence'], ['sheet creation', 'active sheet', 'editable value'], 'server-roundtrip'),
  ...
  coreInteraction('CORE-015', 'Excel-style column width paths share one multi-column transaction surface', 'dimension', 'e2e/spreadsheet.spec.ts', ['interaction', 'mutation', 'undo-redo', 'browser', 'persistence'], ['drag width', 'menu width', 'single transaction'], 'server-roundtrip'),
  ...
  coreInteraction('CORE-017', 'fill handle follows the primary range bottom-right', 'fill', 'e2e/spreadsheet.spec.ts', ['interaction', 'mutation', 'browser', 'persistence'], ['fill handle', 'primary range', 'filled values'], 'server-roundtrip'),
  coreInteraction('CORE-018', 'formula calculation commits through the cloud session and survives reload', 'formula', 'e2e/spreadsheet.spec.ts', ['domain', 'mutation', 'browser', 'persistence'], ['formula result', 'server acknowledgement', 'reload readback'], 'server-roundtrip'),
  ...
  coreInteraction('CORE-020', 'Selection Pane selects, renames, and toggles a drawing through host callbacks', 'drawing', 'e2e/spreadsheet.spec.ts', ['interaction', 'mutation', 'browser', 'persistence'], ['selection pane', 'host callback', 'drawing state'], 'server-roundtrip'),
  ...
  coreInteraction('CORE-023', 'creates a cloud workbook then preserves its session through Backstage', 'backstage', 'e2e/workbook-hub.spec.ts', ['interaction', 'mutation', 'browser', 'persistence'], ['create workbook', 'Backstage session'], 'server-roundtrip'),
  coreInteraction('CORE-024', 'does not turn an unknown unauthenticated workbook into a local session', 'routing', 'e2e/workbook-hub.spec.ts', ['contract', 'interaction', 'browser'], ['unknown route', 'no local session']),
...
export const HOME_BEHAVIOR_CASES: readonly AcceptanceCase[] = [
  ...
  { id: 'H-NUMBER-FORMAT-TRANSACTION', title: 'Number format controls preserve exact custom format semantics', layers: ['domain', 'contract', 'browser', 'persistence'], evidence: ['canonical format value', 'Undo/Redo', 'server readback'] },
...
export const ELASTIC_GRID_CASES: readonly AcceptanceCase[] = [
  ...
  { id: 'G-COLLABORATION-EXTENT-CONVERGENCE', title: 'The command, persisted page, and collaboration projections converge on the same extent', layers: ['contract', 'parity', 'persistence'], evidence: ['canonical manifest', 'remote replay', 'extent equality'] },
...
export const PERSISTENCE_CASES: readonly AcceptanceCase[] = [
  { id: 'G-CLOUD-SAVE-RELOAD', title: 'Cloud save publishes one canonical workbook revision that survives reload', layers: ['contract', 'browser', 'persistence'], evidence: ['server acknowledgement', 'committed revision', 'reload readback'] },

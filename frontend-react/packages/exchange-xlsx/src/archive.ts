...
export { detectPackageFeatures, loadXlsxPackage, parseLoadedXlsx };
...
export function unzipXlsxBase64(base64: string, limits?: Partial<XlsxZipLimits>): Record<string, string> {
  const loaded = loadXlsxPackage(base64ToBytes(base64), limits);
...
export function parseXlsxXmlToSnapshot(files: Record<string, string>): WorkbookSnapshot {
  ...
  const loaded = loadXlsxPackage(zipSync(parts, { level: 0 }));
...
export function exportSnapshotToXlsxBase64(
  ...
  return bytesToBase64(new Uint8Array(exportSnapshotToXlsxPackage(snapshot, {

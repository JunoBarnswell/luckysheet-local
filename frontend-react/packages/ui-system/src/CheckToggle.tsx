...
export const CheckToggle = forwardRef<HTMLInputElement, CheckToggleProps>(function CheckToggle(
  ...
  return (
    <label className={cn('inline-flex min-w-0 items-center gap-2 text-xs text-ink', disabled && 'cursor-not-allowed opacity-50', className)}>
      <span className={cn('relative inline-flex h-4 w-4 shrink-0 items-center justify-center rounded border border-slate-300 bg-white transition-colors has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-accent/30', checkedTone === 'dark' ? 'has-[:checked]:border-black has-[:checked]:bg-black' : 'has-[:checked]:border-accent has-[:checked]:bg-accent')}>

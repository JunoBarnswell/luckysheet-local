...
describe('RibbonShell responsive layout', () => {
  it('keeps one fixed Ribbon geometry at every viewport width', () => {
    ...
    assert.equal(ribbonLayoutModeForWidth(1782), 'wide');
    assert.equal(ribbonLayoutModeForWidth(1400), 'wide');
    assert.equal(ribbonLayoutModeForWidth(1024), 'wide');
    assert.equal(ribbonLayoutModeForWidth(900), 'wide');
    assert.equal(ribbonLayoutModeForWidth(600), 'wide');
  });

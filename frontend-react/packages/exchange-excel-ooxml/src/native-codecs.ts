...
async function untouchedExport(request: NativeDocumentExportTransaction): Promise<NativeDocumentExportResult | undefined> {
  const artifact = request.artifact;
  if (!artifact || artifact.fileName !== request.fileName || artifact.sourceSnapshotHash !== await nativeSnapshotHash(request.snapshot)) return undefined;
  return {

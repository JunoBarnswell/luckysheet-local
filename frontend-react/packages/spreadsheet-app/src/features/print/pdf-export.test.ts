...
describe('print PDF output', () => {
  it('creates a real PDF byte stream in Node', async () => {
    const bytes = createPdfBytes(pages, layout, { title: 'UAT workbook', author: 'Test' });
    assert.equal(new TextDecoder().decode(bytes.subarray(0, 8)), '%PDF-1.7');
    const text = new TextDecoder().decode(bytes);
    assert.match(text, /xref/);
    assert.match(text, /%%EOF/);
    assert.match(text, /UAT workbook/);

    const result = await nodePrintHook.renderPages(pages, layout, { title: 'UAT workbook' });

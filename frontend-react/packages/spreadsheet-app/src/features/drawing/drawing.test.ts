...
describe('drawing feature', () => {
  it('adds, selects, moves, resizes, and reorders unified drawing objects', () => {
    ...
    runtime.execute('drawing.add.image', {
      ...
      payload: { kind: 'image', asset: imageAsset('asset-logo'), altText: 'Logo' },
    });
  ...
  it('applies the same existence reconciliation to every canonical drawing kind', () => {
    ...
    const payloads: Array<{ kind: DrawingObject['kind']; payload: DrawingPayload }> = [
      ...
      { kind: 'chart', payload: { kind: 'chart', chartId: 'payload-kind-3', chartType: 'column', subtype: 'clustered', source: { kind: 'worksheet-ranges', ranges: [] }, elements: { hiddenData: 'show' } } },
  ...
  it('supports typed picture effects with fail-close validation and undo', () => {
    ...
    runtime.execute('drawing.add.image', {
      ...
      payload: { kind: 'image', asset: imageAsset('asset-effects') },
    });
    runtime.clearHistory();

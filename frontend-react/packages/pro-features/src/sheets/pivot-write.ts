...
export function buildPivotWriteback(pivot: PivotModel, sheet: WorksheetModel): PivotWriteResult {
  ...
  const tree = computePivotResult(workbook, pivot); const values = pivot.layout?.values ?? pivot.valueFields; const output: Array<Array<{ value: string | number | boolean | null }>> = [];

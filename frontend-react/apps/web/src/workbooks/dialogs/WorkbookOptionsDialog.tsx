...
export interface WorkbookPreferences {
  ...
  nativeDocumentCompatibility: 'balanced' | 'strict' | 'best-effort';
}
  ...
  return (
    <Dialog closeLabel="关闭文件中心选项" footer={<><Button onClick={onClose} size="sm" variant="ghost">取消</Button><Button onClick={onSave} size="sm" variant="brand">保存设置</Button></>} maxWidth="md" onClose={onClose} open={open} title="文件中心选项" testId="workbook-options-dialog">
      <Stack gap="lg">
        ...
        <Stack gap="xs"><Text as="label" htmlFor="options-document-compatibility" size="sm" weight="medium">原生文档导入兼容级别</Text><Select id="options-document-compatibility" onChange={(event) => patch('nativeDocumentCompatibility', event.target.value as WorkbookPreferences['nativeDocumentCompatibility'])} options={[{ value: 'balanced', label: '平衡（推荐）' }, { value: 'strict', label: '严格保留原生结构' }, { value: 'best-effort', label: '仅按明确规则投影' }]} value={value.nativeDocumentCompatibility} /></Stack>
      </Stack>
    </Dialog>
  );
}

import { AppShell, Box, Button, DropdownMenu, Inline, SidebarShell, Stack, StatePanel, Text } from "@react-sheets/ui-system";
...
function WorkbookRouteGate({ unitId }: { unitId: string }) {
  ...
  if (localState === 'checking') {
    return <Box as="main" className="flex min-h-screen items-center justify-center bg-white p-8"><StatePanel kind="loading" title="正在打开工作簿" description="正在确认本地缓存或云端访问权限。" /></Box>;
  }
  ...
  return <WorkspaceErrorBoundary><WorkspaceApp key={`${unitId}:${authSnapshot.phase}:${authSnapshot.subject ?? 'anonymous'}`} onOpenHub={() => navigate('/workbooks')} /></WorkspaceErrorBoundary>;
}

function WorkspaceApp({ onOpenHub }: { onOpenHub: () => void }) {
  const auth = useAuthSession();
  const { catalog } = useApplicationServices();
  const { session, snapshot: state } = useWorkbookSession({
    initialPhase: getInitialSessionPhase(),
    authTokenProvider: auth.getAccessToken,
  });
  ...
  const exportXlsx = async () => {
    try {
      ...
      session.notify(cause instanceof Error ? cause.message : 'XLSX export failed');
    }
  };
  ...
  const renameWorkbook = async (name: string) => {
    ...
    try {
      await catalog.rename(state.unitId, normalized);
      // The editor's canonical WorkbookModel must receive the same rename
      // mutation so formulas, undo/redo and the active title stay coherent.
      session.renameWorkbook(normalized);
    } catch (cause) {
      session.notify(cause instanceof Error ? cause.message : 'Unable to rename workbook');
    }
  };

  const importXlsx = () => {
    // Import is a catalog command: it must create a new workbook identity and
    // never hydrate an external workbook into the active session.
    navigate('/workbooks?dialog=import');
  };
  ...
  if (backstageOpen) {
    const syncStatus = state.saveState === 'saved'
      ? 'synced'
      : state.saveState === 'saving' || state.saveState === 'calculating'
        ? 'syncing'
    ...
    const closeWorkbook = async () => {
      await session.saveWorkbook('Close workbook');
      onOpenHub();
    };
    const actions = [
    ...
    return (
      <WorkbookBackstageShell
        actions={actions}
        onBack={() => setBackstageOpen(false)}
        onHelp={() => session.notify('帮助：导入会创建新的工作簿；云端与本地文件的状态会显示在文件中心。')}
        onSettings={() => setBackstagePanel('options')}
        readOnly={!state.permissions.editCell}
      ...
      >
        {backstagePanel === 'info' ? (
          <Stack gap="md" className="rounded-xl border border-brand-line bg-white p-6">
            ...
            <Stack gap="xs">
              <Text size="sm">文件名：{state.workbookName}</Text>
              <Text size="sm">工作簿 ID：{state.unitId}</Text>
              <Text size="sm">当前权限：{state.shareRole ?? 'owner'}</Text>
              <Text size="sm">服务端版本：{state.collabRevision}</Text>
              <Text size="sm">待同步操作：{state.pendingChangeSetCount}</Text>
              <Text size="sm">校验和：{state.persistenceChecksum}</Text>
              <Text size="sm">来源：{state.compatibilityReport ? '导入的 XLSX' : '原生工作簿'}</Text>
            </Stack>
          </Stack>
        ) : (
          <Stack gap="md" className="rounded-xl border border-brand-line bg-white p-6">
            <Text size="lg" weight="semibold">选项</Text>
            <Text size="sm">语言</Text>
            <Inline gap="sm">
              <Button onClick={() => setLocale('zh-CN')} size="sm" variant={locale === 'zh-CN' ? 'brand' : 'outline'}>中文</Button>
              <Button onClick={() => setLocale('en-US')} size="sm" variant={locale === 'en-US' ? 'brand' : 'outline'}>English</Button>
            </Inline>
          ...
          >
            {({ close }) => (
              <Stack gap="xs" className="min-w-44">
                <Button size="sm" variant="ghost" className="justify-start" onClick={() => { setBackstagePanel('info'); setBackstageOpen(true); close(); }}>
                  File / 工作簿
                </Button>
...
export default function App() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.pathname === '/') navigate('/workbooks', { replace: true });
  }, []);

  if (route.kind === 'auth-callback' || route.kind === 'auth-silent-renew' || auth.phase === 'loading') {
    return <Box as="main" className="flex min-h-screen items-center justify-center bg-white p-8"><StatePanel kind="loading" title="正在验证登录状态" description="正在建立云端工作簿会话。" /></Box>;
  }
  if (route.kind === 'hub') {
    return <WorkbookHubContainer onOpenWorkbook={(unitId) => navigate(`/workbooks/${encodeURIComponent(unitId)}`)} />;
  }
  if (route.kind === 'not-found') {
    return <Box as="main" className="flex min-h-screen items-center justify-center bg-white p-8"><StatePanel actionLabel="返回工作簿中心" kind="error" title="页面不存在" description={`未找到 ${route.pathname}`} onAction={() => navigate('/workbooks', { replace: true })} /></Box>;
  }
  if (route.kind !== 'workbook') {
    return <Box as="main" className="flex min-h-screen items-center justify-center bg-white p-8"><StatePanel actionLabel="返回工作簿中心" kind="error" title="路由状态无效" description="无法解析当前工作簿路由。" onAction={() => navigate('/workbooks', { replace: true })} /></Box>;
  }

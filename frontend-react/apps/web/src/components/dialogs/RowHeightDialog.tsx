...
export function RowHeightDialog({ open, rowCount, initialHeightPx, onClose, onApply }: RowHeightDialogProps) {
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
      title="Row Height"
      description={`Set the height in points for ${rowCount} selected row${rowCount === 1 ? '' : 's'}. Use Hide Rows for zero-height rows.`}
      maxWidth="sm"
    ...
    >
      <Stack gap="sm">
        ...
        <Inline gap="sm" className="items-center">
          <TextInput aria-label="Row height in points" type="number" min={0.01} max={MAX_EXCEL_ROW_HEIGHT_POINTS} step={0.01} value={value} onChange={(event) => setValue(event.target.value)} />

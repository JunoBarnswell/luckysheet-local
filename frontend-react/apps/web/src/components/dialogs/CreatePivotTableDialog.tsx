...
import type { PivotTaskError } from '@react-sheets/spreadsheet-app';
...
export interface CreatePivotTableDialogProps { open: boolean; sourceRegion: RangeRef; sourceOptions: readonly { id: string; label: string }[]; activeSheetName: string; locale: Locale; pending?: boolean; error?: PivotTaskError; onClose: () => void; onCreate: (request: CreatePivotTableRequest) => void | Promise<void> }
...
export function CreatePivotTableDialog({ open, sourceRegion, sourceOptions, activeSheetName, locale, pending = false, error, onClose, onCreate }: CreatePivotTableDialogProps) {
  ...
  return (
    <Dialog open={open} title={pivotText(locale, 'createTitle')} maxWidth="pivot" bodyClassName="px-3 pb-3 pt-2" onClose={onClose} footer={<Inline gap="sm" className="w-full justify-end"><Button className="min-w-[98px]" size="sm" variant="primary" disabled={pending || (destination === 'existing-sheet' && !targetReference.trim())} data-testid="create-pivot-confirm" onClick={() => onCreate({ sourceId, destination, ...(destination === 'existing-sheet' ? { targetReference: targetReference.trim() } : {}) })}>{pending ? pivotText(locale, 'creating') : pivotText(locale, 'confirm')}</Button><Button className="min-w-[98px]" size="sm" variant="outline" onClick={onClose}>{pivotText(locale, 'cancel')}</Button></Inline>}>
      <Stack gap="md" data-testid="create-pivot-dialog">
        {error ? <Box as="section" role="alert" className="rounded border border-red-300 bg-red-50 px-3 py-2"><Stack gap="xs"><Text size="sm" weight="medium" className="text-red-800">{error.code}</Text><Text size="sm" className="text-red-700">{error.message}</Text><Text size="xs" tone="subtle">{pivotText(locale, 'recovery')}: {error.recovery}</Text></Stack></Box> : null}

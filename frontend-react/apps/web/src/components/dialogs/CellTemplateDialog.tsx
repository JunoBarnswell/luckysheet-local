...
function createEditorConfig(kind: ConfigurableCellEditorKind, values: string): NonNullable<CellStyleTemplate['editor']> {
  switch (kind) {
    case 'datetime':
      return { kind, mode: 'date' };
    case 'combo-box':
      return { kind, items: values.split(',').map((value) => value.trim()).filter(Boolean).map((value) => ({ value })), editable: true };
    case 'mask':
      return { kind, mask: values.trim() || '########' };
    default:
      return { kind };
  }
}
...
export function CellTemplateDialog({ open, templates, onApply, onClose, onRemove, onSave }: CellTemplateDialogProps) {
  ...
  const [editorKind, setEditorKind] = useState<ConfigurableCellEditorKind>('text');
  ...
  const save = () => {
    ...
    if (!trimmed) return;
    const editor = createEditorConfig(editorKind, values);
    onSave({
      id: createTemplateId(trimmed),
      name: trimmed,
      style: { background, textColor, bold: true },
      ...(editor ? { editor } : {}),
      ...(editorKind === 'validation-list' && values.trim() ? {
        dataValidation: {
          type: 'list',
    ...
    <Dialog open={open} title="单元格样式模板" onClose={onClose} maxWidth="lg">
      <Stack gap="md">
        ...
        <Inline gap="sm">
          ...
          <Box className="w-40 shrink-0"><Text size="xs" tone="muted">编辑器</Text><Select aria-label="模板编辑器" sizeVariant="sm" value={editorKind} onChange={(event) => setEditorKind(event.target.value as ConfigurableCellEditorKind)}><option value="text">文本</option><option value="number">数字</option><option value="datetime">日期时间</option><option value="validation-list">验证列表</option><option value="combo-box">组合框</option><option value="checkbox">复选框</option><option value="mask">掩码</option><option value="formula">公式</option><option value="rich-text">富文本</option></Select></Box>
        </Inline>
        {editorKind === 'validation-list' || editorKind === 'combo-box' ? <Box><Text size="xs" tone="muted">列表值（逗号分隔）</Text><TextInput aria-label="模板列表值" value={values} onChange={(event) => setValues(event.target.value)} placeholder="待处理, 进行中, 已完成" /></Box> : editorKind === 'mask' ? <Box><Text size="xs" tone="muted">输入掩码</Text><TextInput aria-label="模板输入掩码" value={values} onChange={(event) => setValues(event.target.value)} /></Box> : null}
        ...
        <Inline gap="sm" className="justify-end"><Button size="sm" variant="ghost" onClick={onClose}>关闭</Button><Button size="sm" variant="primary" disabled={!name.trim() || ((editorKind === 'validation-list' || editorKind === 'combo-box') && values.split(',').map((value) => value.trim()).filter(Boolean).length === 0)} onClick={save}>保存模板</Button></Inline>

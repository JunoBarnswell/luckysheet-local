...
function resolveValueSortId(field: AreaItem, selected: Readonly<Record<string, string>>, valueFields: readonly PivotValueField[]): string {
  const persisted = field.placement?.sort?.by === 'value' ? field.placement.sort.valueId : undefined;
  const available = new Set(valueFields.map((value) => value.valueId));
  const selectedId = selected[field.fieldId];
  if (selectedId && available.has(selectedId)) return selectedId;
  if (persisted && available.has(persisted)) return persisted;
  return valueFields.length === 1 ? valueFields[0]!.valueId : '';
}
                ...
                <Inline gap="xs" className="p-1">
                   <Stack gap="xs" className="min-w-48 p-1">
                     ...
                     <Inline gap="xs">
                      ...
                      {onSort && area !== 'values' && area !== 'filters' ? (() => {
                        const valueSortId = resolveValueSortId(field, valueSortFieldIds, valueFields);
                        return <Stack gap="xs" className="border-t border-slate-100 pt-1"><Text size="xs" weight="semibold">{pivotText(locale, 'sortBy')}</Text><Inline gap="xs"><Button size="xs" variant="ghost" onClick={() => { onSort(field.fieldId, { direction: 'ascending', by: 'label' }); close(); }}>{pivotText(locale, 'ascending')}</Button><Button size="xs" variant="ghost" onClick={() => { onSort(field.fieldId, { direction: 'descending', by: 'label' }); close(); }}>{pivotText(locale, 'descending')}</Button></Inline><Select aria-label={`${field.name} ${pivotText(locale, 'sortField')}`} sizeVariant="sm" disabled={disabled || valueFields.length === 0} value={valueSortId} onChange={(event) => setValueSortFieldIds((current) => ({ ...current, [field.fieldId]: event.target.value }))}><option value="" disabled>{pivotText(locale, 'sortField')}</option>{valueFields.map((value) => <option key={value.valueId} value={value.valueId}>{value.displayName ?? value.fieldId}</option>)}</Select><Inline gap="xs"><Button size="xs" variant="ghost" disabled={!valueSortId} onClick={() => { if (valueSortId) { onSort(field.fieldId, { direction: 'ascending', by: 'value', valueId: valueSortId }); close(); } }}>{pivotText(locale, 'valueAscending')}</Button><Button size="xs" variant="ghost" disabled={!valueSortId} onClick={() => { if (valueSortId) { onSort(field.fieldId, { direction: 'descending', by: 'value', valueId: valueSortId }); close(); } }}>{pivotText(locale, 'valueDescending')}</Button></Inline><Button size="xs" variant="ghost" onClick={() => { onSort(field.fieldId, undefined); close(); }}>{pivotText(locale, 'clearSort')}</Button></Stack>;
                      })() : null}

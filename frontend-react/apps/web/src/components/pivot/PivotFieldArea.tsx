...
import type { PivotFieldArea as Area, PivotFieldDefinition } from './types';
...
export interface PivotFieldAreaProps {
  ...
  onSort?: (fieldId: string, direction: 'ascending' | 'descending') => void;
...
export function PivotFieldArea({ area, disabled = false, fieldIds, fields, filterSelections = {}, onDrop, onFilter, onGroup, onMoveByKeyboard, onRemove, onSort }: PivotFieldAreaProps) {
  const items: AreaItem[] = fieldIds.map((id, index) => ({ ...(fields.find((field) => field.id === id) ?? { id, label: id, type: 'text' as const }), index }));
  return (
      ...
      <FieldDropZone
        ...
        renderItem={(field) => (
          ...
          >
            ...
            <Text size="xs" weight="medium" className="min-w-0 flex-1 truncate">{field.label}</Text>
            <DropdownMenu
              align="right"
              trigger={<Button aria-label={`Keyboard menu for ${field.label}`} icon="more-horizontal" iconOnly size="xs" variant="ghost" />}
            >
              {({ close }) => (
                <Inline gap="xs" className="p-1">
                   <Stack gap="xs" className="min-w-48 p-1">
                     ...
                     {onFilter && field.values?.length ? <Stack gap="xs" className="border-t border-slate-100 pt-1"><Text size="xs" weight="semibold">Filter values</Text>{field.values.map((value) => { const selected = filterSelections[field.id] ?? field.values ?? []; return <CheckToggle key={value} label={value} checked={selected.includes(value)} onChange={(event) => onFilter(field.id, event.target.checked ? [...selected, value] : selected.filter((item) => item !== value))} />; })}</Stack> : null}
                   </Stack>
                </Inline>
              )}
            </DropdownMenu>

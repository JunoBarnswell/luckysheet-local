...
export function PivotHeaderFilterPopover({ currentFilters, currentSort, field, locale, memberOptions, onApply, onClose, scope = 'field', valueFields, x, y }: PivotHeaderFilterPopoverProps) {
  ...
  const apply = () => {
    ...
    if (sort?.by === 'value' && !valueFields.some((value) => value.valueId === sort.valueId)) return;
    if (mode === 'top-items') {
  ...
  return (
    <Panel className="absolute z-40 w-[300px] rounded-none border border-[#a9a9a9] bg-white shadow-[2px_4px_10px_rgba(0,0,0,0.2)]" style={{ left: x, top: y }}>
      <Stack gap="none">
        ...
        {showSortOptions ? <Stack gap="xs" className="border-b border-[#d7d7d7] px-3 py-2"><Inline gap="xs"><Select aria-label={pivotText(locale, 'sortField')} sizeVariant="sm" value={sort?.by ?? 'label'} onChange={(event) => {
          if (event.target.value === 'value') {
            const valueId = sort?.by === 'value' && valueFields.some((value) => value.valueId === sort.valueId) ? sort.valueId : valueFields[0]?.valueId;
            if (valueId) setSort({ direction: sort?.direction ?? 'ascending', by: 'value', valueId });
          } else setSort({ direction: sort?.direction ?? 'ascending', by: 'label' });
        }}><option value="label">{pivotText(locale, 'labelFilter')}</option><option value="value" disabled={valueFields.length === 0}>{pivotText(locale, 'valueFilter')}</option></Select><Button size="xs" variant="ghost" onClick={() => setSort(undefined)}>{pivotText(locale, 'clearSort')}</Button></Inline>{sort?.by === 'value' ? <Select aria-label={pivotText(locale, 'sortField')} sizeVariant="sm" value={sort.valueId} onChange={(event) => setSort({ direction: sort.direction, by: 'value', valueId: event.target.value })}><option value="" disabled>{pivotText(locale, 'sortField')}</option>{valueFields.map((value) => <option key={value.valueId} value={value.valueId}>{value.label}</option>)}</Select> : null}</Stack> : null}

...
import type { PivotDefinition, PivotFieldDefinition, PivotPanelCallbacks } from './types';

export interface PivotActionsProps { definition: PivotDefinition; fields: readonly PivotFieldDefinition[]; callbacks: PivotPanelCallbacks; disabled?: boolean; }

export function PivotActions({ callbacks, definition, disabled = false, fields }: PivotActionsProps) {
  ...
  return (
    <Box as="section" aria-label="Pivot configuration actions" className="border-t border-line/80 pt-3">
      ...
      <Inline gap="xs" className="flex-wrap">
        ...
        <Select aria-label="Pivot layout" disabled={disabled} sizeVariant="sm" value={definition.layout} onChange={(event) => callbacks.onLayoutChange(event.target.value as PivotDefinition['layout'])}><option value="compact">Compact</option><option value="outline">Outline</option><option value="tabular">Tabular</option></Select>
        <Button disabled={disabled || !controlField} icon="eye" size="xs" variant="outline" onClick={() => callbacks.onExpandedChange(controlField, !definition.expandedFieldIds.includes(controlField))}>Expand state</Button>
        ...
        <Button disabled={disabled || !controlField} icon="sliders" size="xs" variant="outline" onClick={() => callbacks.onSlicerChange(controlField, !definition.slicers.includes(controlField))}>Slicer</Button>

  ...
  return (
    <Box as="section" aria-label="Pivot configuration actions" className="border-t border-line/80 pt-3">
      ...
      <Inline gap="xs" className="flex-wrap">
        ...
        <Select aria-label="Pivot control field" disabled={disabled || fields.length === 0} sizeVariant="sm" value={controlField} onChange={(event) => setControlField(event.target.value)}>{fields.map((field) => <option key={field.id} value={field.id}>{field.name}</option>)}</Select>

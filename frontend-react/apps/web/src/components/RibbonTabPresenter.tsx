...
export function RibbonTabPresenter({ tab, locale, layout, renderCommand }: RibbonTabPresenterProps) {
  ...
  return (
    <Inline gap="none" className="h-[102px] w-max min-w-full flex-nowrap items-start overflow-visible" data-testid={`ribbon-groups-${tab}`}>
      {groups.map((group, groupIndex) => {
        ...
        return (
          <React.Fragment key={group.id}>
            ...
            <Stack gap="none" className={`h-[102px] shrink-0 justify-between overflow-hidden px-1 ${ribbonGroupWidthClass(group.id)}`}>

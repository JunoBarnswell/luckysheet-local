...
import { chartSeriesSupportsErrorBars, chartSeriesSupportsTrendlines, defaultChartSubtype, type ChartDrawingPayload } from '@react-sheets/core-model';
...
export function ChartSeriesEditor({ series, chartType, subtype, selectedSeriesId, onChange, onAdd, onMaterialize, canAdd, allowEmpty }: ChartSeriesEditorProps) {
  ...
  return <Stack gap="sm">
    ...
    {series.map((entry, index) => {
      const seriesType = chartType === 'combo' ? entry.value.chartType ?? 'column' : chartType;
      const stockSubtype = entry.value.subtype ?? subtype;
      return <Box key={entry.value.id ?? index} className={`rounded-lg border p-3 ${entry.value.id === selectedSeriesId ? 'border-emerald-500 bg-emerald-50/50' : 'border-slate-200 bg-white'}`}>
      <Stack gap="sm">
        <Inline className="justify-between"><Text size="xs" weight="semibold">系列 {index + 1}</Text><Inline gap="xs">
          ...
          <Button aria-label={`删除系列 ${index + 1}`} icon="trash" iconOnly size="xs" variant="ghost" disabled={series.length === 1 && !allowEmpty} onClick={() => onChange(series.filter((_, position) => position !== index))} />
        </Inline></Inline>
        ...
        {!['scatter', 'bubble', 'stock'].includes(seriesType) ? <TextInput aria-label={`系列 ${index + 1} 范围`} value={entry.range} placeholder="数值范围，例如 B2:B20" onChange={event => update(index, item => ({ ...item, range: event.target.value }))} /> : null}
        ...
        {seriesType === 'scatter' || seriesType === 'bubble' ? <Stack gap="xs">{(['xRange', 'yRange', ...(seriesType === 'bubble' ? ['sizeRange' as const] : [])] as const).map(key => <TextInput key={key} aria-label={`系列 ${index + 1} ${key}`} value={entry[key]} placeholder={key === 'xRange' ? 'X 值范围' : key === 'yRange' ? 'Y 值范围' : '气泡大小范围'} onChange={event => update(index, item => ({ ...item, [key]: event.target.value }))} />)}</Stack> : null}
        {seriesType === 'stock' ? <Stack gap="xs">
          {stockSubtype.includes('volume') ? <TextInput aria-label={`系列 ${index + 1} Volume 范围`} value={entry.stockVolumeRange} placeholder="Volume 范围" onChange={event => update(index, item => ({ ...item, stockVolumeRange: event.target.value }))} /> : null}
          {stockSubtype.includes('open') ? <TextInput aria-label={`系列 ${index + 1} Open 范围`} value={entry.stockOpenRange} placeholder="Open 范围" onChange={event => update(index, item => ({ ...item, stockOpenRange: event.target.value }))} /> : null}
          <TextInput aria-label={`系列 ${index + 1} High 范围`} value={entry.stockHighRange} placeholder="High 范围" onChange={event => update(index, item => ({ ...item, stockHighRange: event.target.value }))} />
          <TextInput aria-label={`系列 ${index + 1} Low 范围`} value={entry.stockLowRange} placeholder="Low 范围" onChange={event => update(index, item => ({ ...item, stockLowRange: event.target.value }))} />
          <TextInput aria-label={`系列 ${index + 1} Close 范围`} value={entry.stockCloseRange} placeholder="Close 范围" onChange={event => update(index, item => ({ ...item, stockCloseRange: event.target.value }))} />
        </Stack> : null}
        ...
        {chartSeriesSupportsTrendlines(seriesType) ? <><Inline gap="xs"><Button size="xs" variant="secondary" onClick={() => update(index, item => ({ ...item, value: { ...item.value, trendlines: [...(item.value.trendlines ?? []), { id: crypto.randomUUID(), type: 'linear', displayEquation: true, displayRSquared: true }] } }))}>添加线性趋势线</Button></Inline>
        {entry.value.trendlines?.length ? <Button size="xs" variant="ghost" onClick={() => update(index, item => ({ ...item, value: { ...item.value, trendlines: [] } }))}>清除趋势线（{entry.value.trendlines.length}）</Button> : null}</> : null}
        {chartSeriesSupportsErrorBars(seriesType) ? <Box className="space-y-2 rounded-md border border-slate-200 bg-slate-50 p-2"><Text size="xs" weight="medium">误差线</Text>

      ...
      <Box className="mx-auto max-w-4xl">
        ...
        <Panel>
          ...
          <PanelBody>
            ...
            {!loading && !error && workbooks.length > 0 ? <Stack gap="xs">{workbooks.map((workbook) => <Inline key={workbook.unitId} gap="md" className="items-center justify-between rounded-lg border border-line bg-white px-4 py-3"><Stack gap="none" className="min-w-0"><Text size="sm" weight="semibold" className="truncate">{workbook.name}</Text><Text size="xs" tone="subtle">Revision {workbook.revision} · {new Date(workbook.updatedAt).toLocaleString()}</Text></Stack><Button size="sm" variant="secondary" onClick={() => window.location.assign(`/workbooks/${encodeURIComponent(workbook.unitId)}`)}>Open</Button></Inline>)}</Stack> : null}
          </PanelBody>
        </Panel>
      </Box>
    </Box>

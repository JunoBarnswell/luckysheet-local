...
function drawCanonicalMap(context: CanvasRenderingContext2D, categories: string[], series: CanvasChartSeries | undefined, left: number, top: number, width: number, height: number): void {
  ...
  if (categories.length === 0 || values.length === 0) {
    context.fillStyle = '#b91c1c'; context.font = '11px Segoe UI, sans-serif'; context.textAlign = 'left'; context.textBaseline = 'top'; context.fillText('Map requires region and numeric value fields', left + 8, top + 8, Math.max(10, width - 16)); return;
  }
  const max = Math.max(...values); const min = Math.min(...values); const columns = Math.min(5, Math.max(1, Math.ceil(Math.sqrt(categories.length)))); const rows = Math.ceil(categories.length / columns); const cellWidth = width / columns; const cellHeight = height / rows;
  categories.forEach((category, index) => { const column = index % columns; const row = Math.floor(index / columns); const value = values[index] ?? 0; const ratio = max === min ? 0.5 : (value - min) / (max - min); const red = Math.round(219 - ratio * 110); const green = Math.round(234 - ratio * 120); const blue = Math.round(254 - ratio * 60); const x = left + column * cellWidth; const y = top + row * cellHeight; context.fillStyle = `rgb(${red},${green},${blue})`; context.fillRect(x + 1, y + 1, Math.max(1, cellWidth - 2), Math.max(1, cellHeight - 2)); context.fillStyle = ratio > 0.58 ? '#ffffff' : '#1e293b'; context.font = '10px Segoe UI, sans-serif'; context.textAlign = 'center'; context.textBaseline = 'middle'; context.fillText(category, x + cellWidth / 2, y + cellHeight / 2 - 6, Math.max(8, cellWidth - 6)); context.fillText(String(value), x + cellWidth / 2, y + cellHeight / 2 + 7, Math.max(8, cellWidth - 6)); });
}

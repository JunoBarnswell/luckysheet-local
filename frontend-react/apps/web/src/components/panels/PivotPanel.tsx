...
import { defaultPivotFieldArea, PIVOT_FIELD_AREAS, PIVOT_FIELD_PANE_LAYOUTS, shouldDeferPivotLayoutUpdates, type PivotFieldArea as Area, type PivotFieldPaneLayout, type PivotManualFilterState, type PivotPanelCallbacks, type PivotPanelSlots, type PivotPanelState, type PivotSlicerControl, type PivotTimelineControl } from '../pivot/pivot-contract';
...
export function PivotPanel({ activePivotId, callbacks, fieldCatalog: suppliedFields, locale, onClose, pivot, pivotList = [], slicerControls = [], slots, state, timelineControls = [] }: PivotPanelProps) {
  ...
  // Excel exposes all four placement containers as the default report layout.
  // Users can still switch to stacked/side-by-side modes explicitly, but the
  // first entry into a PivotTable must not hide Rows and Values below the fold.
  const [fieldPaneLayout, setFieldPaneLayout] = useState<PivotFieldPaneLayout>('areas-2x2');
  const [fieldPaneSplit, setFieldPaneSplit] = useState(45);
  useEffect(() => {
    setDraft(pivot ? cloneLayout(pivot.layout) : null);
    setDelayUpdate(pivot ? shouldDeferPivotLayoutUpdates(pivot) : false);
    setDirty(false);
    // A new Pivot context always opens in the usable Excel-style workspace.
    // Do not carry an earlier "areas only" choice forward and hide the field
    // catalog that owns checkbox and drag interactions.
    setFieldPaneLayout('areas-2x2');
    setFieldPaneSplit(45);
  }, [pivot?.id]);
  ...
  const workspaceClass = sideBySide
    ? 'flex flex-row'
    : showFields && showAreas ? 'grid' : 'flex flex-col';
  const workspaceStyle = !sideBySide && showFields && showAreas
    ? { gridTemplateRows: `minmax(96px, ${fieldPaneSplit}fr) 6px minmax(112px, ${100 - fieldPaneSplit}fr)` }
    : undefined;
  ...
  return (
    <Panel className="flex h-full max-h-full min-h-0 flex-col overflow-hidden rounded-none border-0 bg-white shadow-none" data-testid="pivot-field-list">
      <Inline gap="sm" className="h-10 w-full shrink-0 justify-between px-3">
        <Text size="lg" weight="bold">{pivotText(locale, 'fieldsTitle')}</Text>
        {onClose ? <Button aria-label={pivotText(locale, 'close')} icon="x" iconOnly size="sm" variant="ghost" onClick={onClose} /> : null}
      </Inline>
      ...
      <Stack gap="xs" className="min-h-0 flex-1 overflow-hidden px-3 pb-1">
        <Inline gap="sm" className="h-8 shrink-0 justify-between border-b border-slate-100">
        ...
        <Box className={`${workspaceClass} min-h-0 min-w-0 flex-1 overflow-hidden gap-2`} style={workspaceStyle}>
          {showFields ? (
          ...
          {showFields && showAreas ? (
            <Box
              ...
              tabIndex={0}
              className={`${sideBySide ? 'h-full w-2 cursor-col-resize' : 'h-1.5 w-full cursor-row-resize'} shrink-0 touch-none rounded bg-slate-100 hover:bg-blue-100`}
          ...
          {showAreas ? (
            <Box className="min-h-0 min-w-0 flex flex-1 flex-col overflow-hidden">
              ...
              <Box className={`${fieldPaneLayout === 'areas-1x4' ? 'flex flex-col' : 'grid grid-cols-2'} h-full min-h-0 flex-1 gap-1`} style={fieldPaneLayout === 'areas-1x4' ? undefined : { gridTemplateRows: 'repeat(2, minmax(0, 1fr))' }}>
        ...
        <Inline gap="xs" className="h-9 shrink-0 border-t border-[#d0d0d0] bg-white pt-1">
          ...
          <DropdownMenu align="right" trigger={<Button aria-label={pivotText(locale, 'advancedFields')} icon="plus" iconOnly size="xs" variant="ghost" />}><Box className="w-[22rem] p-3"><PivotCalculatedEditor locale={locale} fields={fields} calculatedFields={layout.calculatedFields ?? []} calculatedItems={layout.calculatedItems ?? []} disabled={disabled} onFieldsChange={(next) => applyLayout({ ...cloneLayout(layout), calculatedFields: next })} onItemsChange={(next) => applyLayout({ ...cloneLayout(layout), calculatedItems: next })} /></Box></DropdownMenu>
          <Button size="xs" variant={dirty ? 'primary' : 'outline'} disabled={!delayUpdate || !dirty || applying} onClick={() => { void applyDraft(); }}>{applying ? pivotText(locale, 'applyingLayout') : pivotText(locale, 'update')}</Button>
        </Inline>
        {delayUpdate && dirty ? <Text size="xs" tone="subtle" className="shrink-0 truncate text-amber-700">{applying ? pivotText(locale, 'applyingLayout') : pivotText(locale, 'pendingLayout')}</Text> : null}
        {slots?.statusSummary ? <Text size="xs" tone="subtle" className="shrink-0 truncate">{slots.statusSummary}</Text> : null}
      </Stack>
    </Panel>
  );
}

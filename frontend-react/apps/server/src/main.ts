...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'GET' && /^\/api\/v1\/workbooks\/[^/]+\/revisions\/\d+$/.test(url.pathname)) {
      ...
      if (!result) { sendJson(response, 404, { code: 'NOT_FOUND', message: 'Revision not found' }); return; }
      sendJson(response, 200, result);
      return;
    }
    if (request.method === 'POST' && url.pathname === '/api/v1/workbooks') {
      const input = JSON.parse(await readBody(request)) as { name?: string; snapshot?: ReturnType<typeof createDefaultSnapshot> };
...
webSocketServer.on('connection', (socket) => {
  ...
  socket.send(encodeMessage({ type: 'presence.updated', unitId: '', actorId: 'server', state: { status: 'connected' } }));
  socket.on('message', (raw) => {
    try {
      ...
      if (message.type === 'changeset.submit') {
        ...
        socket.send(encodeMessage({ type: 'changeset.ack', operationId: message.payload.operationId, revision }));
        const revisionMessage = encodeMessage({ type: 'revision.created', payload: message.payload, revision });
        for (const client of clients) {
          if (client !== socket && client.readyState === client.OPEN) client.send(revisionMessage);
        }
      }
    } catch (error) {
      socket.send(encodeMessage({ type: 'changeset.reject', operationId: 'unknown', error: { code: 'VALIDATION_ERROR', message: error instanceof Error ? error.message : 'Invalid message' } }));
    }
  });
});
